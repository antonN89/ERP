...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	СведенияОВнешнейОбработке.Вставить("Версия"						, "9.4");
	...
	СведенияОВнешнейОбработке.Вставить("АвторскиеПраваНаОбработку"	, "2019");
	СведенияОВнешнейОбработке.Вставить("ДомашняяСтраницаОбработки"	, "http://www.infostart.ru");
...
Процедура _Настройки_ФормыОбработки(Настройки)
	...
	Настройки.Вставить("ИмяФормыСтруктурыПодчиненности"				, ОбработкаПолучитьИмяФормы("СвязанныеДокументы"));
...
Функция ОбъектБДСформироватьТРеквизитыДобавитьСтроку(мдОбъектаБД, ОбъектБД, ИмяРеквизита, Представление, БлокируемыеРеквизиты, НеРедактируемыеРеквизиты, Проверяемый, Длина = Неопределено)
	Перем Мас, СтрокаТРеквизиты, РезультатРеквизит, Тип, Реквизит;
	Перем КоллекцияМД, ОбъектАдресации;
			...
			Иначе
				Если Мас.Найти(ИмяРеквизита) = Неопределено Тогда
					Если Метаданные.ОбщиеРеквизиты.Содержит(мдОбъектаБД) Тогда	// Общий реквизит.
						...
						КонецЕсли;
					Иначе
						КоллекцияМД = "Реквизиты";
						Если Строка(ТипЗнч(ОбъектБД)) = "Задача" Тогда
							ОбъектАдресации =  мдОбъектаБД.РеквизитыАдресации.Найти(ИмяРеквизита);
							Если НЕ ОбъектАдресации = Неопределено Тогда
								КоллекцияМД = "РеквизитыАдресации";
							Иначе
								КоллекцияМД = "Реквизиты";
							КонецЕсли;
						КонецЕсли;
						Если Тип = "Строка" Тогда
							Длина = мдОбъектаБД[КоллекцияМД][ИмяРеквизита].Тип.КвалификаторыСтроки.Длина;
							Длина = ?(Длина = 0, "неограниченная", Длина);
							СтрокаТРеквизиты.Тип = СтрокаТРеквизиты.Тип + " (" + Длина + ")";
							Если Длина = "неограниченная" Тогда
								СтрокаТРеквизиты.Проверяемый = Ложь;
							КонецЕсли;
						Иначе
							Если мдОбъектаБД[КоллекцияМД][ИмяРеквизита].Тип.КвалификаторыЧисла.РазрядностьДробнойЧасти > 0 Тогда
								СтрокаТРеквизиты.Тип = СтрокаТРеквизиты.Тип + " (" + мдОбъектаБД[КоллекцияМД][ИмяРеквизита].Тип.КвалификаторыЧисла.Разрядность + "." + мдОбъектаБД[КоллекцияМД][ИмяРеквизита].Тип.КвалификаторыЧисла.РазрядностьДробнойЧасти + ")";
							Иначе
								СтрокаТРеквизиты.Тип = СтрокаТРеквизиты.Тип + " (" + мдОбъектаБД[КоллекцияМД][ИмяРеквизита].Тип.КвалификаторыЧисла.Разрядность + ")";
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
...
Функция GUIDВосстановитьОбъектПоТипуИГУИД(БитыйОбъектТип, БитыйОбъектGUID, БитыйОбъектДатаИВремя, БитыйОбъектТекст) Экспорт
	...
	КонецЕсли;
	ОбъектВоссоздаваемыйСсылка = ОбъектМенеджер.ПолучитьСсылку(БитыйОбъектGUID);
	ОбъектВоссоздаваемый.УстановитьСсылкуНового(ОбъектМенеджер.ПолучитьСсылку(БитыйОбъектGUID));
	
	Если НЕ ОбъектМД = Неопределено И ЕстьРеквизитОбъекта1С("Дата", ОбъектВоссоздаваемыйСсылка) Тогда
		ОбъектВоссоздаваемый.Дата = БитыйОбъектДатаИВремя;
	КонецЕсли;
	
	Если НЕ ОбъектМД = Неопределено И ЕстьРеквизитОбъекта1С("Организация", ОбъектВоссоздаваемыйСсылка) Тогда
		Попытка
...
Функция ЗначениеСсылочногоТипа(Значение) Экспорт
	...
	//bercut060819 расширения методом ТипВсеСсылки() не поддерживаются
...
// 08.04.2016. Дополняет переданную в качестве первого параметра строку символами слева\справа до заданной длины и возвращает ее
...
Функция ДополнитьСтроку(Знач Строка, Знач ДлинаСтроки, Знач Символ = "0", Знач Режим = "Слева")
	Перем КоличествоСимволовНадоДобавить, СтрокаДляДобавления;
...
// 08.04.2016. Формирует строку повторяющихся символов заданной длины
//
...
Функция ПолучитьСтатистикуДокументовНаСервере(ТаблицаСтатистика, СтатДатаНач, СтатДатаКон, СтатОрганизация) Экспорт
	Перем ИндексДокумента, Синоним, Комментарий, ЕстьРеквизитОрганизация, Периодичность, ПроведениеРазрешено, КоличествоДвижений;
	...
	Для Каждого мдДокумента ИЗ Метаданные.Документы Цикл
		...
		//Синоним = СтрЗаменить(МДдокумента.Синоним, """", """""");
		Синоним = СтрЗаменить(МДдокумента.Синоним, """", "");
...
Процедура ОчисткаКоллекцииДвиженийДокумента(ДокументОбъект)
	...
	Для Каждого ДвиженияДокумента ИЗ ДокументОбъект.Движения Цикл
...
////////////////////////////////////////////////////////////////////////////////
// ВКЛАДКА "ДОПОЛНИТЕЛЬНО". "УДАЛЕНИЕ ДОКУМЕНТОВ".
//
...
Функция ДокументУдалениеПолучитьСписокУдаляемыхДокументовНаСервере(ДокументУдалениеТСписокВидов, ТаблицаДокументыУдаление, ДатаНачала, ДатаОкончания, ДокументУдалениеОрганизации, ДокументУдалениеСклады, ДокументУдалениеМагазины, ДокументУдалениеКассы, ДокументУдалениеКассыККМ, ДокументУдалениеПодразделения, ДокументУдалениеПодразделенияОрганизаций, ПоискПоСвязанным, ПоискПоНоменклатуре, ДокументУдалениеГруппаНоменклатуры) Экспорт
	...
	Перем Запрос, ЗапросФормирование, Результат;
	...
	Если ЗапросФормирование И ПоискПоНоменклатуре И ЗначениеЗаполнено(ДокументУдалениеГруппаНоменклатуры) Тогда
...
////////////////////////////////////////////////////////////////////////////////
// ВКЛАДКА "ДОПОЛНИТЕЛЬНО". "УДАЛЕНИЕ ДОКУМЕНТОВ (ГРУБАЯ СИЛА)".
//

Функция ДокументУдалениеГСЗаполнитьСписокВидовДокументов(ДокументСписокВидов, Организация) Экспорт
	Перем мдДокумента, НоваяСтрока, ЯвляетсяОснованиемДля;
	...
	Для Каждого мдДокумента ИЗ Метаданные.Документы Цикл
		...
		Если НЕ ЕстьРеквизитОбъектаМетаданных1С("Организация", мдДокумента) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ДокументУдалениеГСПроизвестиУдалениеДокументовНаСервере(Знач ДокументУдалениеГСДействие, Знач ТаблицаСписокВидовДокументов, ДокументУдалениеГСПериод, Знач ДокументУдалениеГСОрганизации, ДокументУдалениеГСКомментарий) Экспорт
	Перем Запрос, РезультатЗапроса, Выборка, ДатаНачала, ДатаОкончания;
	Перем Организация, мдДокумента, ДокументСсылка, ДокументОбъект, ПроведениеРазрешено, ДвижениеДокумента, МассивСтрок, ОбрабатываемыйДокумент;
	Перем КВоДокТИПА, КВоДокВСЕГО, Сообщение;
	
	УстановитьПривилегированныйРежим(Истина);
	
	ДатаНачала = НачалоДня(ДокументУдалениеГСПериод.ДатаНачала);
	ДатаОкончания = КонецДня(ДокументУдалениеГСПериод.ДатаОкончания);
	Если ДатаОкончания = КонецДня(Дата("00010101")) Тогда
		ДатаОкончания = КонецДня(ТекущаяДата());
	КонецЕсли;
	
	КВоДокВСЕГО = 0;
	ДокументУдалениеГСКомментарий = "";
	Для Каждого мдДокумента ИЗ Метаданные.Документы Цикл
		...
		МассивСтрок = ТаблицаСписокВидовДокументов.НайтиСтроки(Новый Структура ("Значение", мдДокумента.Имя));
		Если МассивСтрок.Количество() = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		ОбрабатываемыйДокумент = МассивСтрок[0].Пометка;
		Если НЕ ОбрабатываемыйДокумент Тогда
			Продолжить;
		КонецЕсли;
		...
		|ИЗ
		|	Документ." + мдДокумента.Имя + " КАК Документ
		|ГДЕ
		|	Документ.Организация В(&СписокОрганизаций)
		|	И Документ.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания";
		
		Запрос.УстановитьПараметр("СписокОрганизаций"	, ДокументУдалениеГСОрганизации);
		Запрос.УстановитьПараметр("ДатаНачала"			, ДатаНачала);
		Запрос.УстановитьПараметр("ДатаОкончания"		, ДатаОкончания);
		...
		Пока Выборка.Следующий() Цикл
			...
			Если НЕ ДокументУдалениеГСДействие = "Удалить" И ДокументСсылка.ПометкаУдаления Тогда
				Продолжить;
			КонецЕсли;
			...
			// Отмена проведения документа / Удаление движений документа.
			Если ЕстьРеквизитОбъекта1С("ОбменДанными", ДокументОбъект) Тогда	// "ГРУБАЯ СИЛА".
				ДокументОбъект.ОбменДанными.Загрузка = Истина;	// Не все объекты имеют свойство "ОбменДанными".
			КонецЕсли;
			Если ПроведениеРазрешено Тогда
				Попытка
			...
			// Удаление документа.
			Попытка
				Если ДокументУдалениеГСДействие = "Удалить" Тогда
					ДокументОбъект.Удалить();
				Иначе
					ДокументОбъект.УстановитьПометкуУдаления(Истина);
				КонецЕсли;
				КВоДокТИПА = КВоДокТИПА + 1;
				КВоДокВСЕГО = КВоДокВСЕГО + 1;
			Исключение
				Сообщить("Не удалось удалить документ: " + ДокументСсылка);
			КонецПопытки;
			
		КонецЦикла;
		Сообщение = "" + ?(ДокументУдалениеГСДействие = "Удалить", "Удалено ", "Помечено на удаление ") + КВоДокТИПА + " документов типа " + мдДокумента.Имя + "." + Символы.ПС;
		ДокументУдалениеГСКомментарий = ДокументУдалениеГСКомментарий + Сообщение;
		
	КонецЦикла;
	Сообщение = "" + ?(ДокументУдалениеГСДействие = "Удалить", "Удалено ВСЕГО ", "Помечено на удаление ВСЕГО ") + КВоДокВСЕГО + " документов." + Символы.ПС;
	ДокументУдалениеГСКомментарий = ДокументУдалениеГСКомментарий + Сообщение;
