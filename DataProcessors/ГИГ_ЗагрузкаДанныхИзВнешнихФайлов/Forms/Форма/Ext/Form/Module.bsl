...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Задача: [№ 9975], [#Добавить кнопку на форму]
	ДоработкаФормы();
	// - [Rineco], [Киселев А.] [26.07.2021]
	...
	//++Гольм А.А. (Гигабайт) 18.03.2019 21:24:49
	ГИГ_ПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка);
	//--Гольм А.А. (Гигабайт) 18.03.2019 21:24:53
КонецПроцедуры
...
// + [Rineco], [Киселев А.] [26.07.2021] 
// Задача: [№ 9975], [#КнопкаФормы и обработчик]
&НаСервере
Процедура ДоработкаФормы()
	...
	КнопкаСвернуть = Элементы.Вставить("СвернутьСтроки",Тип("КнопкаФормы"),Элементы.Товары.КоманднаяПанель,Элементы.ТоварыСкопироватьСтроки);
...
Процедура СвернутьСтроки(Команда)
	 ...
	 СвернутьСтрокиСервер();
		
КонецПроцедуры

&НаСервере
Процедура  СвернутьСтрокиСервер()
	
	ТаблицаДляСвертки = Объект.Товары.Выгрузить();
	// + [Rineco], [Киселев А.Н.] [06.10.2021] 
	// Задача: [№ 19053], [#Ошибка после обновления]
	
	// Было:
	//ТаблицаДляСвертки.Свернуть("Номенклатура, НоменклатураПартнера, Характеристика, Упаковка, Цена, СтавкаНДС, ПроцентРучнойСкидки, НомерГТД, УпаковочныйЛист, МинимальныйОстаток, МаксимальныйОстаток, Производитель, НоменклатураПроизводителей ","Количество,КоличествоУпаковок, СуммаРучнойСкидки,Сумма,СуммаНДС,СуммаСНДС");
	// Стало:
	ТаблицаДляСвертки.Свернуть("Номенклатура, НоменклатураПоставщика, Характеристика, Упаковка, Цена, СтавкаНДС, ПроцентРучнойСкидки, НомерГТД, УпаковочныйЛист, МинимальныйОстаток, МаксимальныйОстаток, Производитель, НоменклатураПроизводителей ","Количество,КоличествоУпаковок, СуммаРучнойСкидки,Сумма,СуммаНДС,СуммаСНДС");
	// - [Rineco], [Киселев А.Н.] [06.10.2021]
	
	Объект.Товары.Очистить();
	Объект.Товары.Загрузить(ТаблицаДляСвертки);
...
Процедура СопоставитьДанныеТабличногоДокументаСДаннымиИБ()
	...
	Объект.Товары.Очистить();
	//++ НВА: 08.11.2017:	
	УточнениеНоменклатурыПроизводителей.Очистить();
	//-- НВА
	...
	//++ НВА: 03.11.2017:	
	// + [Rineco], [Киселев А.] [24.11.2021] 
	Запрос = Новый Запрос;
	// Задача: [№ 21837], [#Оптимизация]
	
	// Было:
	//Запрос.Текст = "ВЫБРАТЬ
	//|	НомХар.Родитель КАК Родитель,
	//|	НомХар.ЭтоГруппа КАК ЭтоГруппа,
	//|	НомХар.Артикул КАК Артикул,
	//|	НомХар.Наименование КАК Наименование,
	//|	НомХар.Ссылка КАК НоменклатураПартнера,
	//|	НомХар.ПометкаУдаления КАК ПометкаУдаления,
	//|	НомХар.Номенклатура КАК Номенклатура,
	//|	НомХар.Производитель КАК Производитель,
	//|	НомХар.Характеристика КАК Характеристика
	//|ПОМЕСТИТЬ ВрНомХар
	//|ИЗ
	//|	Справочник.ГИГ_НоменклатураПроизводителей КАК НомХар
	//|
	//|ГДЕ
	//|	НЕ НомХар.ЭтоГруппа
	//|	И НЕ НомХар.ПометкаУдаления
	//|;
	//|
	//|////////////////////////////////////////////////////////////////////////////////
	//|ВЫБРАТЬ
	//|	ВрНомХар.Родитель КАК Родитель,
	//|	ВрНомХар.ЭтоГруппа КАК ЭтоГруппа,
	//|	ВрНомХар.Артикул КАК Артикул,
	//|	ВрНомХар.Артикул КАК АртикулПоставщика,
	//|	ВрНомХар.Артикул КАК АртикулПоиск,
	//|	ВрНомХар.Артикул КАК АртикулПоставщикаПоиск,
	//|	ВрНомХар.Наименование КАК НаименованиеПолноеПоиск,
	//|	ВрНомХар.Наименование КАК НаименованиеПолное,
	//|	ВрНомХар.Наименование КАК НаименованиеПоиск,
	//|	ВрНомХар.Наименование КАК Наименование,
	//|	ВрНомХар.НоменклатураПартнера КАК НоменклатураПроизводителей,
	//|	ВрНомХар.ПометкаУдаления КАК ПометкаУдаления,
	//|	ВрНомХар.Номенклатура КАК Номенклатура,
	//|	ВрНомХар.Производитель КАК Производитель,
	//|	ВрНомХар.Характеристика КАК Характеристика
	//|ИЗ
	//|	ВрНомХар КАК ВрНомХар
	//|";
	// Стало:
	Запрос.Текст = "ВЫБРАТЬ 
	|	НомХар.Родитель КАК Родитель,
	|	НомХар.ЭтоГруппа КАК ЭтоГруппа,
	|	НомХар.Артикул КАК Артикул,
	|	НомХар.Артикул КАК АртикулПоставщика,
	|	НомХар.Артикул КАК АртикулПоиск,
	|	НомХар.Артикул КАК АртикулПоставщикаПоиск,
	|	НомХар.Наименование КАК НаименованиеПолноеПоиск,
	|	НомХар.Наименование КАК НаименованиеПолное,
	|	НомХар.Наименование КАК НаименованиеПоиск,
	|	НомХар.Наименование КАК Наименование,
	|	НомХар.Ссылка КАК НоменклатураПроизводителей,
	|	НомХар.ПометкаУдаления КАК ПометкаУдаления,
	|	НомХар.Номенклатура КАК Номенклатура,
	|	НомХар.Производитель КАК Производитель,
	|	НомХар.Характеристика КАК Характеристика
	...
	// Было:
	//ТаблицаНоменклатурыПроизводителей.Загрузить(Результат.Выгрузить());
	// Стало:
	ТаблицаНоменклатурыПроизводителейТест = Результат.Выгрузить();
	ТаблицаНоменклатурыПроизводителейТест.Индексы.Добавить("АртикулПоиск,Наименование,Производитель");
	// - [Rineco], [Киселев А.] [24.11.2021]
	...
	Пока ЗаполненыНоменклатураКодАртикулШтрихкод Цикл
		...
		//++ НВА: 03.11.2017: поиск по номенклатуре производителей
		НоваяСтрокаТовары.ТребуетУточнения = Ложь;
		Если МассивПодобраннойНоменклатуры.Количество() > 0 Тогда
			...
			НоваяСтрокаТовары.ТребуетУточнения = Истина;
			Для Каждого СтрНоменклатура Из МассивПодобраннойНоменклатуры Цикл
				СтрокаУточнения = УточнениеНоменклатурыПроизводителей.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаУточнения, НоваяСтрокаТовары);
				ЗаполнитьЗначенияСвойств(СтрокаУточнения, СтрНоменклатура);
				Если СтрНоменклатура = МассивПодобраннойНоменклатуры[0] Тогда
					СтрокаУточнения.ВыбратьСтроку = Истина;
					СтрокаУточнения.КодСтроки = НоваяСтрокаТовары.КодСтроки;
				Иначе
					СтрокаУточнения.ВыбратьСтроку = Ложь;
					СтрокаУточнения.КодСтроки = "";
				КонецЕсли;
				СтрокаУточнения.Артикул = СтрНоменклатура.Номенклатура.Артикул;
			КонецЦикла;
		КонецЕсли;
		Если ЗначениеЗаполнено(НоваяСтрока.Артикул) Тогда
			СтруктураПоиска = Новый Структура;
			СтруктураПоиска.Вставить("АртикулПоиск", НоваяСтрока.Артикул);
			Если ЗначениеЗаполнено(НоваяСтрока.НоменклатураНаименование) Тогда 
				СтруктураПоиска.Вставить("Наименование", НоваяСтрока.НоменклатураНаименование);
			КонецЕсли;	
			Если ЗначениеЗаполнено(НоваяСтрока.Производитель) Тогда 
				ТекущийПроизводитель = Справочники.Производители.НайтиПоНаименованию(НоваяСтрока.Производитель, Истина);
				Если ЗначениеЗаполнено(ТекущийПроизводитель) Тогда
					СтруктураПоиска.Вставить("Производитель", ТекущийПроизводитель);
				КонецЕсли;
			КонецЕсли;	
			// + [Rineco], [Киселев А.] [24.11.2021] 
			// Задача: [№ 21837], [#Оптимизация]
			...
			// Было:
			//НайденныеСтроки = ТаблицаНоменклатурыПроизводителей.НайтиСтроки(СтруктураПоиска);
			// Стало:
			НайденныеСтроки = ТаблицаНоменклатурыПроизводителейТест.НайтиСтроки(СтруктураПоиска);
			// - [Rineco], [Киселев А.] [24.11.2021]
			
			КоличествоНайденныхСтрок = НайденныеСтроки.Количество();
			КоличествоПодобраннойНоменклатуры = МассивПодобраннойНоменклатуры.Количество();
			Если КоличествоНайденныхСтрок > 0 Тогда
				Если КоличествоПодобраннойНоменклатуры = 0 Тогда
					Для Каждого СтрНоменклатураПроизводителя Из НайденныеСтроки Цикл
						Если СтрНоменклатураПроизводителя = НайденныеСтроки[0] Тогда
							ЗаполнитьЗначенияСвойств(НоваяСтрокаТовары, СтрНоменклатураПроизводителя);
							НоваяСтрокаТовары.Артикул = НоваяСтрокаТовары.Номенклатура.Артикул;
							НоваяСтрокаТовары.АртикулПроизводителя = СтрНоменклатураПроизводителя.Артикул;
							НоваяСтрокаТовары.ТребуетУточнения = Истина;							
						КонецЕсли;
						СтрокаУточнения = УточнениеНоменклатурыПроизводителей.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаУточнения, СтрНоменклатураПроизводителя);
						СтрокаУточнения.АртикулПроизводителя = СтрНоменклатураПроизводителя.Артикул;
						СтрокаУточнения.Артикул = СтрНоменклатураПроизводителя.Номенклатура.Артикул;						
						СтрокаУточнения.Количество = НоваяСтрокаТовары.Количество;
						СтрокаУточнения.Цена = НоваяСтрокаТовары.Цена;
						Если СтрНоменклатураПроизводителя = НайденныеСтроки[0] Тогда							
							СтрокаУточнения.ВыбратьСтроку = Истина;
							СтрокаУточнения.КодСтроки = НоваяСтрокаТовары.КодСтроки;
						КонецЕсли;
					КонецЦикла;
				Иначе
					ТребуетсяВыполнитьУточнение = Ложь;
					Для Каждого СтрНоменклатураПроизводителя Из НайденныеСтроки Цикл
						Если ЗначениеЗаполнено(СтрНоменклатураПроизводителя.Номенклатура) Тогда
							ЕстьСопоставление = Ложь;
							Для Каждого СтрМассивПодобраннойНоменклатуры Из МассивПодобраннойНоменклатуры Цикл
								Если СтрМассивПодобраннойНоменклатуры.Номенклатура = СтрНоменклатураПроизводителя.Номенклатура Тогда
									ЕстьСопоставление = Истина;
									Если НЕ ЗначениеЗаполнено(СтрМассивПодобраннойНоменклатуры.НоменклатураПроизводителей) Тогда
										СтрМассивПодобраннойНоменклатуры.НоменклатураПроизводителей = СтрНоменклатураПроизводителя.НоменклатураПроизводителей;
										СтрМассивПодобраннойНоменклатуры.Производитель = СтрНоменклатураПроизводителя.Производитель;									
										СтрМассивПодобраннойНоменклатуры.АртикулПроизводителя = СтрНоменклатураПроизводителя.Артикул;
										ПараметрыОтбора = Новый Структура;
										ПараметрыОтбора.Вставить("Номенклатура", СтрМассивПодобраннойНоменклатуры.Номенклатура);
										НайденныеСтрокиУточнениеНоменклатурыПроизводителей = УточнениеНоменклатурыПроизводителей.НайтиСтроки(ПараметрыОтбора);
										Для Каждого СтрНайденныеСтрокиУточнениеНоменклатурыПроизводителей Из НайденныеСтрокиУточнениеНоменклатурыПроизводителей Цикл
											СтрНайденныеСтрокиУточнениеНоменклатурыПроизводителей.НоменклатураПроизводителей = СтрНоменклатураПроизводителя.НоменклатураПроизводителей;
											СтрНайденныеСтрокиУточнениеНоменклатурыПроизводителей.Производитель = СтрНоменклатураПроизводителя.Производитель;									
											СтрНайденныеСтрокиУточнениеНоменклатурыПроизводителей.АртикулПроизводителя = СтрНоменклатураПроизводителя.Артикул;
										КонецЦикла;
									КонецЕсли;
								КонецЕсли;
							КонецЦикла;
							Если НЕ ЕстьСопоставление Тогда
								ТребуетсяВыполнитьУточнение = Истина;
							КонецЕсли;
						Иначе
							ТребуетсяВыполнитьУточнение = Истина;							
						КонецЕсли;
					КонецЦикла;
					Если ТребуетсяВыполнитьУточнение Тогда
						Для Каждого Стр Из МассивПодобраннойНоменклатуры Цикл
							Если Стр <> МассивПодобраннойНоменклатуры[0] Тогда
								Стр.КодСтроки = "";
							КонецЕсли;	
							ПараметрыОтбора = Новый Структура;
							ПараметрыОтбора.Вставить("Номенклатура", Стр.Номенклатура);
							НайденныеСтрокиУточнениеНоменклатурыПроизводителей = УточнениеНоменклатурыПроизводителей.НайтиСтроки(ПараметрыОтбора);
							Если НайденныеСтрокиУточнениеНоменклатурыПроизводителей.Количество() = 0 Тогда							
								СтрокаУточнения = УточнениеНоменклатурыПроизводителей.Добавить();														
								ЗаполнитьЗначенияСвойств(СтрокаУточнения, Стр);
								СтрокаУточнения.Артикул = Стр.Номенклатура.Артикул;
								СтрокаУточнения.Количество = НоваяСтрокаТовары.Количество;
								СтрокаУточнения.Цена = НоваяСтрокаТовары.Цена;
							Иначе
								Для Каждого СтрНайденныеСтроки Из НайденныеСтрокиУточнениеНоменклатурыПроизводителей Цикл
									ЗаполнитьЗначенияСвойств(СтрНайденныеСтроки, Стр);
									СтрокаУточнения.Артикул = Стр.Номенклатура.Артикул;
									СтрокаУточнения.Количество = НоваяСтрокаТовары.Количество;
									СтрокаУточнения.Цена = НоваяСтрокаТовары.Цена;
								КонецЦикла;
						...
						Для Каждого Стр Из НайденныеСтроки Цикл
							ПараметрыОтбора = Новый Структура;
							ПараметрыОтбора.Вставить("НоменклатураПроизводителей", Стр.НоменклатураПроизводителей);
							НайденныеСтрокиУточнениеНоменклатурыПроизводителей = УточнениеНоменклатурыПроизводителей.НайтиСтроки(ПараметрыОтбора);
							Если НайденныеСтрокиУточнениеНоменклатурыПроизводителей.Количество() = 0 Тогда							
								СтрокаУточнения = УточнениеНоменклатурыПроизводителей.Добавить();														
								ЗаполнитьЗначенияСвойств(СтрокаУточнения, Стр);
								СтрокаУточнения.АртикулПроизводителя = Стр.Артикул;
								СтрокаУточнения.Артикул = Стр.Номенклатура.Артикул;
								СтрокаУточнения.Количество = НоваяСтрокаТовары.Количество;
								СтрокаУточнения.Цена = НоваяСтрокаТовары.Цена;
							Иначе
	...
	// Было:
	//	//++ НВА: 09.11.2017:
	//ТаблицаНоменклатурыПроизводителей.Очистить();
	////-- НВА

	// Стало:
	ТаблицаНоменклатурыПроизводителейТест.Очистить();
	// - [Rineco], [Киселев А.] [24.11.2021]
...
Процедура ИнициализироватьТабличныйДокумент()
	...
	МакетЗаполненияПоВнешнемуИсточнику = Обработки.ГИГ_ЗагрузкаДанныхИзВнешнихФайлов.ПолучитьМакет("МакетЗагрузкиДанных");
...
//{{20200930 ГлазуновДВ 
	НовыйНомерГТД.РегистрационныйНомер = НомерГТД;
//}}20200930 ГлазуновДВ 	
...
Процедура ПроверитьДублиАртикулов()
	...
	//Если есть дубли:
	//1) Выводим окно с отбором
	//2) Удаляем лишнее
	//3) Открываем страницу
	
	Если УточнениеНоменклатурыПроизводителей.Количество() > 0 Тогда
		
		ПараметрыФормыКорректировки = Новый Структура;
		ПараметрыФормыКорректировки.Вставить("ТаблицаУточнения", ПолучитьТЧУточнениеНоменклатурыПроизводителей());
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ПослеЗакрытияФормыУточненияНоменклатурыПроизводителей", ЭтотОбъект);
		
		ОткрытьФорму("Обработка.ГИГ_ЗагрузкаДанныхИзВнешнихФайлов.Форма.ФормаКорректировкиНоменклатурыПроизводителей", ПараметрыФормыКорректировки, ЭтаФорма,,,,ОписаниеОповещения);	
...
&НаКлиенте
Процедура ПослеЗакрытияФормыУточненияНоменклатурыПроизводителей(Результат, Параметр) Экспорт
	
	Если ТипЗнч(Результат) = Тип("Массив") Тогда
		
		Для Каждого Стр Из Результат Цикл
			
			Если ЗначениеЗаполнено(Стр.КодСтроки) И НЕ Стр.ВыбратьСтроку Тогда
				
				ПараметрыОтбора = Новый Структура;
				ПараметрыОтбора.Вставить("КодСтроки", Стр.КодСтроки);
				НайденныеСтроки = Объект.Товары.НайтиСтроки(ПараметрыОтбора); 
				
				Для Каждого ТекСтрТовары Из НайденныеСтроки Цикл 
					Объект.Товары.Удалить(ТекСтрТовары);
				КонецЦикла;
				
			ИначеЕсли НЕ ЗначениеЗаполнено(Стр.КодСтроки) И Стр.ВыбратьСтроку Тогда
				
				НоваяСтрокаТовары = Объект.Товары.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрокаТовары, Стр);
				НоваяСтрокаТовары.ЗагружаемыйАртикул = Стр.АртикулПроизводителя;
				...
				Если НЕ НеПересчитыватьСуммовыеПоказатели Тогда
					...
					ЕстьСкидки = ЗначениеЗаполнено(НоваяСтрокаТовары.СуммаРучнойСкидки) ИЛИ ЗначениеЗаполнено(НоваяСтрокаТовары.ПроцентРучнойСкидки);
					
					Если НЕ ЗначениеЗаполнено(НоваяСтрокаТовары.СуммаНДС) ИЛИ ЕстьСкидки Тогда
						СтруктураДействий.Вставить("ПересчитатьСуммуНДС", СтруктураПересчетаСуммы);
					КонецЕсли;
					
					Если НЕ ЗначениеЗаполнено(НоваяСтрокаТовары.СуммаСНДС) ИЛИ ЕстьСкидки Тогда
						СтруктураДействий.Вставить("ПересчитатьСуммуСНДС", СтруктураПересчетаСуммы);
					КонецЕсли; 
					
					Если НЕ ЗначениеЗаполнено(НоваяСтрокаТовары.Сумма) ИЛИ ЕстьСкидки Тогда
						СтруктураДействий.Вставить("ПересчитатьСумму");
					КонецЕсли;
					
					Если НЕ ЗначениеЗаполнено(НоваяСтрокаТовары.СуммаРучнойСкидки) И ЗначениеЗаполнено(НоваяСтрокаТовары.ПроцентРучнойСкидки) Тогда
						СтруктураДействий.Вставить("ПересчитатьСуммуРучнойСкидки");
					КонецЕсли;
					
					Если ЗначениеЗаполнено(НоваяСтрокаТовары.СуммаРучнойСкидки) И НЕ ЗначениеЗаполнено(НоваяСтрокаТовары.ПроцентРучнойСкидки) Тогда
						СтруктураДействий.Вставить("ПересчитатьПроцентРучнойСкидки");
					КонецЕсли;
					
					Если ЗначениеЗаполнено(НоваяСтрокаТовары.СуммаРучнойСкидки) ИЛИ ЗначениеЗаполнено(НоваяСтрокаТовары.ПроцентРучнойСкидки) Тогда
						СтруктураДействий.Вставить("ПересчитатьСуммуСУчетомРучнойСкидки", Новый Структура("Очищать", Ложь));
					КонецЕсли;
			...
			ИначеЕсли ЗначениеЗаполнено(Стр.КодСтроки) И Стр.ВыбратьСтроку Тогда
				...
				Для Каждого ТекущаяСтрока Из НайденныеСтроки Цикл 
					...
					ЗаполнитьЗначенияСвойств(ТекущаяСтрока, Стр,,"НомерГТД,СтранаПроисхождения");  //bercut130520 задача 200 
...
&НаСервере
Функция ПолучитьТЧУточнениеНоменклатурыПроизводителей()
	
	Возврат ОбщегоНазначения.ТаблицаЗначенийВМассив(УточнениеНоменклатурыПроизводителей.Выгрузить())
...
Процедура СоздатьНоменклатуруПоНоменклатуреПроизводителя(Команда)
	...
	Для Каждого Стр Из Элементы.Товары.ВыделенныеСтроки Цикл
		...
		ТекущаяСтрока = Объект.Товары.НайтиПоИдентификатору(Стр);
...
//++Гольм А.А. (Гигабайт) 18.03.2019 21:25:02
&НаСервере
Процедура ГИГ_ПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка)
	Объект.ЗагружатьНоменклатуруПроизводителей = ?(Параметры.Свойство("ЗагружатьНоменклатуруПроизводителей"), Параметры.ЗагружатьНоменклатуруПроизводителей, Ложь);
КонецПроцедуры //--Гольм А.А. (Гигабайт) 18.03.2019 21:25:05