...
Процедура Рин1_ОбработкаВыбораПосле(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ИсточникВыбора.ИмяФормы = "Обработка.Рин1_РаботаСТиповымНаборомМатериалов.Форма.ПодборМатериалов" Тогда
		
		ЗаполнитьТабличнуюЧастьТоварамиИзХранилища(ВыбранноеЗначение.АдресТоваровВХранилище,ВыбранноеЗначение.ИмяТЧ,"1");
...
&НаСервере
Процедура ЗаполнитьТабличнуюЧастьТоварамиИзХранилища(АдресТоваровВХранилище,знач ИмяТЧ, ДополнительныеПараметры = Неопределено)
	...
	Для Каждого СтрокаТоваров Из ТоварыИзХранилища Цикл
		...
		// + [Rineco], [Киселев А.Н.] [03.11.2021] 
		// Задача: [№ 21245], [#Доработка]
		
		// Было:
		//ЗаполнитьЗначенияСвойств(СтрокаТЧТовары, СтрокаТоваров,,"НомерСтроки");	
		// Стало:
		ЗаполнитьЗначенияСвойств(СтрокаТЧТовары, СтрокаТоваров,,"НомерСтроки, КодСтроки");	
		// - [Rineco], [Киселев А.Н.] [03.11.2021]
		...
		Если ДополнительныеПараметры <> Неопределено Тогда
			СтрокаТЧТовары.Склад = СтрокаТоваров.Склад;
		КонецЕсли;
...
// + [Rineco], [Киселев А.Н.] [29.10.2021] 
// Задача: [№ 21245], [#Вычислить максимальный код ТЧ Обеспечение]
&НаСервере
Процедура Рин1_ПередЗаписьюНаСервереПосле(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	
	Если Не Отказ Тогда 
		
		ТекущийОбъект.МаксимальныйКодСтрокиОбеспечение = ПолучитьМаксимальныйКодСтрокиТЧОбеспечение(ТекущийОбъект.ОбеспечениеМатериаламиИРаботами.Выгрузить());	
...
// Задача: [№ 21245], [#Вычислить максимальный код ТЧ Обеспечение]
&НаСервере
Функция ПолучитьМаксимальныйКодСтрокиТЧОбеспечение(ТабличнаяЧасть)
	
	ПараметрыЗапроса = Новый Структура;
	ПараметрыЗапроса.Вставить("ТабличнаяЧасть",ТабличнаяЧасть);
	ТаблицаДанных = ОбщегоНазначенияУТ.ЗапросВыполнитьВыгрузить(ТекстЗапросаМаксимальныйКодСтроки(),ПараметрыЗапроса);	
	
	Если ТаблицаДанных.Количество() = 0 Тогда
		Возврат 0;
	КонецЕсли;
	
	Возврат ТаблицаДанных[0].КодСтроки;
...
// Задача: [№ 21245], [#Вычислить максимальный код ТЧ Обеспечение]
&НаСервере
Функция ТекстЗапросаМаксимальныйКодСтроки()
	Возврат "ВЫБРАТЬ 
	        ...
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ
	        |	МАКСИМУМ(ВТ_Таблица.КодСтроки) КАК КодСтроки
	        |ИЗ
...
// + [Rineco], [Киселев А.] [14.12.2021] [Log_Diff][№ ], [#ТиповойНаборМатериалов] 
&НаКлиенте
Процедура Рин1_ПодобратьПоТиповымНаборамПосле(Команда)
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ЭтапПроизводства",Объект.Ссылка);
	
	ОткрытьФорму("Обработка.Рин1_РаботаСТиповымНаборомМатериалов.Форма.ПодборМатериалов",ПараметрыФормы,ЭтаФорма,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
