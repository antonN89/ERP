...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	//++Гольм А.А. (Гигабайт) 18.03.2019 21:29:41
	ГИГ_ОбработкаВыбораПосле(ВыбранноеЗначение, ИсточникВыбора);
	//--Гольм А.А. (Гигабайт) 18.03.2019 21:29:43
КонецПроцедуры
...
Функция ПолучениеТаблицыЗапасовВФоне()
	...
	//bercut200520
	ТаблицаСпособыОбеспеченияВсе = Объект.СпособыОбеспечения.Выгрузить(,
	"СпособОбеспечения, ФормироватьПлановыеЗаказы, ДатаПоставки");
	
	Если не ТаблицаСпособыОбеспечения.Количество() = ТаблицаСпособыОбеспеченияВсе.Количество() Тогда
		ИПН = Неопределено;
		Для каждого Элемент Из КомпоновщикНастроекДляОтборов.Настройки.Отбор.Элементы Цикл
			Если Элемент.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СпособОбеспечения") Тогда
				ИПН = Элемент.идентификаторпользовательскойнастройки;	
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		Если не ИПН = Неопределено Тогда
			
			СЗ_СпособыОбеспечения = Новый СписокЗначений;
			Для каждого Строка Из ТаблицаСпособыОбеспечения Цикл
				СЗ_СпособыОбеспечения.Добавить(Строка.СпособОбеспечения);
			КонецЦикла;
			
			Для каждого Элемент Из КомпоновщикНастроекДляОтборов.ПользовательскиеНастройки.Элементы Цикл
				Если Элемент.идентификаторпользовательскойнастройки = ИПН Тогда
					//Если не Элемент.Использование Тогда
						Элемент.ПравоеЗначение = СЗ_СпособыОбеспечения;
...
Процедура РассчитатьСтрокуТаблицыЗаказыКОбеспечениюНаКлиенте(СпособОбеспечения)
	...
	//bercut210619
	Отбор = Новый Структура("Номенклатура, Характеристика, Склад, Назначение, СпособОбеспечения,
	|Заказ, ДатаОтгрузки, Требуется, ПодразделениеПолучатель, ДатаОтгрузкиЗаказа, СкладОтгрузкиЗаказа, СхемаОбеспечения");
	//
...
Процедура СохранитьНастройки()
	...
	Настройки = Новый Структура("ФильтрНеобеспеченные, РазместитьЗаказыНаОтгрузкуВСформированныхЗаказахНаПоставку, ОтборПоСкладу, ОтборПоСегменту, ОтборПоНоменклатуре");
...
//++Гольм А.А. (Гигабайт) 18.03.2019 21:28:54
&НаСервере
Процедура ГИГ_ПрограммноеДобавлениеЭлементовНаСервере()
	
КонецПроцедуры //--Гольм А.А. (Гигабайт) 18.03.2019 21:29:05

//++Гольм А.А. (Гигабайт) 18.03.2019 21:27:01
&НаКлиенте
Процедура ГИГ_СхемаОбеспеченияЗаказыНачалоВыбораВместо(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	...
	ОткрытьФорму("Справочник.СхемыОбеспечения.Форма.ГИГ_ВыборСхемыПоСкладу",Новый Структура("Склад,Номенклатура,Характеристика",ОтборСклад,ОтборНоменклатура,ОтборХарактеристикаНоменклатуры),Элементы["СхемаОбеспеченияЗаказы"]);
КонецПроцедуры //--Гольм А.А. (Гигабайт) 18.03.2019 21:27:06

//++Гольм А.А. (Гигабайт) 18.03.2019 21:27:11
&НаКлиенте
Процедура ГИГ_СхемаОбеспеченияЗаказыОбработкаВыбораВместо(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	
	ДатаРасчета = ТекущаяДата();
	ТД = Элемент.Родитель.ТекущиеДанные;
	
	ДанныеНоменклатуры = Новый Структура("Номенклатура,Характеристика");
	ЗаполнитьЗначенияСвойств(ДанныеНоменклатуры,ТД);
	
	СтруктураСроки = ГИГ_МодульВызовСервера.ГИГ_ПолучитьСрокиОбеспеченияПоСхеме(ВыбранноеЗначение,ДатаРасчета,ТД.Склад,ДанныеНоменклатуры);
	
	ЗаполнитьЗначенияСвойств(ТД,СтруктураСроки);
	//ЗаполнитьПоСхеме(ЭтаФорма);
	ЗаполнитьЗначенияСвойств(ТД,ГИГ_ОбеспечениеСервер.ПолучитьСпособОбеспеченияПоСхеме(ТД.СхемаОбеспечения,ТД.Склад));
	
КонецПроцедуры //--Гольм А.А. (Гигабайт) 18.03.2019 21:27:14

//++Гольм А.А. (Гигабайт) 18.03.2019 21:27:17
&НаКлиенте
Процедура ГИГ_СохранитьНастройкиПосле(Команда)
	
	СтруктураНастроек = Новый Структура;
	СтруктураНастроек.Вставить("ОтборПоСкладу",ОтборПоСкладу);
	СтруктураНастроек.Вставить("ОтборПоЦеховымКладовым",ОтборПоЦеховымКладовым);
	СтруктураНастроек.Вставить("ОтборПоНазначению",ОтборПоНазначению);
	СтруктураНастроек.Вставить("ОтборПоНаправлениюДеятельности",ОтборПоНаправлениюДеятельности);
	СтруктураНастроек.Вставить("ОтборПоСегменту",ОтборПоСегменту);
	СтруктураНастроек.Вставить("ОтборПоНоменклатуре",ОтборПоНоменклатуре);
	СтруктураНастроек.Вставить("ОтборПоМарке",ОтборПоМарке);
	СтруктураНастроек.Вставить("ОтборПоТоварнойКатегории",ОтборПоТоварнойКатегории);
	СтруктураНастроек.Вставить("ОтборПоПодразделению",ОтборПоПодразделению);
	СтруктураНастроек.Вставить("ОтборПоТипуОбеспечения",ОтборПоТипуОбеспечения);
	СтруктураНастроек.Вставить("ОтборПоИсточникуОбеспечения",ОтборПоИсточникуОбеспечения);
	СтруктураНастроек.Вставить("ПроизвольныйОтбор",ПолучитьПроизвольныйОтбор());
	СтруктураНастроек.Вставить("ПростыеОтборы", НЕ Элементы.СтраницыОтборов.ТекущаяСтраница = Элементы.СтраницаПроизвольныеОтборы);
	
	//++Шерстюк Ю.Ю. 28.06.2021 Задача 10215 форма отсутствует вызывать будем типовую
	////ФормаНастроек = ОткрытьФорму("Обработка.ОбеспечениеПотребностей.Форма.ГИГ_Настройки",Новый Структура("РежимСохранения",Истина));
	////ФормаНастроек.АвтоЗаголовок = Ложь;
	////ФормаНастроек.Заголовок = "Сохранение настройки";
	////ФормаНастроек.АдресНастроек = ПоместитьВоВременноеХранилище(СтруктураНастроек,ФормаНастроек.УникальныйИдентификатор);
	ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекСохранить("Обработка.ОбработкаОбеспечениеПотребностей", "Пользовательская", СтруктураНастроек);
	////--Шерстюк Ю.Ю.

КонецПроцедуры //--Гольм А.А. (Гигабайт) 18.03.2019 21:27:22

//++Гольм А.А. (Гигабайт) 18.03.2019 21:27:26
&НаСервере
Функция ПолучитьПроизвольныйОтбор()

	Возврат КомпоновщикНастроекДляОтборов.ПолучитьНастройки();

КонецФункции //--Гольм А.А. (Гигабайт) 18.03.2019 21:27:33

//++Гольм А.А. (Гигабайт) 18.03.2019 21:27:37
&НаКлиенте
Процедура ГИГ_ВосстановитьНастройкиПосле(Команда)
	//++Шерстюк Ю.Ю. 28.06.2021 Задача 10215
	//ФормаНастроек = ОткрытьФорму("Обработка.ОбеспечениеПотребностей.Форма.ГИГ_Настройки",Новый Структура("РежимСохранения",Ложь),ЭтаФорма);
	//ФормаНастроек.АвтоЗаголовок = Ложь;
	//ФормаНастроек.Заголовок = "Восстановление настройки";
	//ВосстановитьНастройкиНаСервере();
	ВосстановитьНастройкиНаСервере();
	//--Шерстюк Ю.Ю.
КонецПроцедуры //--Гольм А.А. (Гигабайт) 18.03.2019 21:27:41

&НаСервере
Процедура ВосстановитьНастройкиНаСервере()
	//++Шерстюк Ю.Ю. 28.06.2021 Задача 10215
	ЗначениеНастроек = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("Обработка.ОбработкаОбеспечениеПотребностей", "Пользовательская");

	Если ТипЗнч(ЗначениеНастроек) = Тип("Структура") Тогда
          КомпоновщикНастроекДляОтборов.ЗагрузитьФиксированныеНастройки(ЗначениеНастроек.ПроизвольныйОтбор);
		//КомпоновщикНастроекДляОтборов.ЗагрузитьПользовательскиеНастройки(ЗначениеНастроек.ПользовательскиеНастройки);
		КомпоновщикНастроекДляОтборов.Восстановить(СпособВосстановленияНастроекКомпоновкиДанных.ПроверятьДоступность);
...
//++Гольм А.А. (Гигабайт) 18.03.2019 21:27:44
&НаКлиенте
Процедура ГИГ_ОбработкаВыбораПосле(ВыбранноеЗначение, ИсточникВыбора)
	Если ТипЗнч(ИсточникВыбора) = Тип("УправляемаяФорма") Тогда
		Если ИсточникВыбора.ИмяФормы = "Обработка.ОбеспечениеПотребностей.Форма.ГИГ_Настройки" Тогда
			ЗаполнитьЗначенияСвойств(ЭтаФорма,ВыбранноеЗначение);
			Если ВыбранноеЗначение.Свойство("ПроизвольныйОтбор") И ТипЗнч(ВыбранноеЗначение.ПроизвольныйОтбор) = Тип("НастройкиКомпоновкиДанных") Тогда
				КомпоновщикНастроекДляОтборов.ЗагрузитьНастройки(ВыбранноеЗначение.ПроизвольныйОтбор);
				Если ВыбранноеЗначение.ПростыеОтборы Тогда
					Элементы.СтраницыОтборов.ТекущаяСтраница = Элементы.СтраницаПростыеОтборы;
				Иначе
					Элементы.СтраницыОтборов.ТекущаяСтраница = Элементы.СтраницаПроизвольныеОтборы;
				КонецЕсли;
		...
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры //--Гольм А.А. (Гигабайт) 18.03.2019 21:27:50
