...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	ВыбранныйМенеджер = Справочники.Пользователи.ПустаяСсылка();
	ВыбранныйМенеджерИспользовать = Ложь;
//{{20200428 ГлазуновДВ Добавлено для автоматической подстановки отбора по полю "Менеджер"
	Для Каждого ЭлементОтбора из НастройкиОтчета.Отбор.Элементы Цикл
		Если типЗНЧ(ЭлементОтбора) <> Тип("ЭлементОтбораКомпоновкиДанных") Тогда
			Продолжить;
		КонецЕсли;
		Если ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("АналитикаОборотов.Менеджер") Тогда
			ВыбранныйМенеджерИспользовать = ЭлементОтбора.Использование;
			ВыбранныйМенеджер = ЭлементОтбора.ПравоеЗначение;
			ЭлементОтбора.Использование = Ложь;
			ЭлементОтбора.ПравоеЗначение = Справочники.Пользователи.ПустаяСсылка();
		    //ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		    //ЭлементОтбора.Использование = Истина;
		    //ЭлементОтбора.ПравоеЗначение = ПараметрыСеанса.ТекущийПользователь;
			Прервать;
		КонецЕсли;
		//Если ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("АналитикаОборотов.Заказ") Тогда
		//	ЭлементОтбора.Использование = Ложь;
		//    //ЭлементОтбора.Использование = Истина;
		//    //ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		//	//ЭлементОтбора.ПравоеЗначение = Справочники.Пользователи.ПустаяСсылка();
	...
	НовоеПолеОтбора = Новый ПолеКомпоновкиДанных("АналитикаОборотов.Заказ.Договор.Менеджер");
	Найден = Ложь;
	Для Каждого ЭлементОтбора из НастройкиОтчета.Отбор.Элементы Цикл
		Если типЗНЧ(ЭлементОтбора) <> Тип("ЭлементОтбораКомпоновкиДанных") Тогда
			Продолжить;
		КонецЕсли;
		Если ЭлементОтбора.ЛевоеЗначение = НовоеПолеОтбора Тогда
			Найден = Истина;
		КонецЕсли;
	КонецЦикла;
	Если НЕ Найден Тогда
	    ЭлементОтбора = НастройкиОтчета.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	    ЭлементОтбора.ЛевоеЗначение = НовоеПолеОтбора;
	    ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	    ЭлементОтбора.Использование = ВыбранныйМенеджерИспользовать;//Истина;
	    //ЭлементОтбора.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный;
	    ЭлементОтбора.ПравоеЗначение = ВыбранныйМенеджер;
	Иначе
	    ЭлементОтбора = НастройкиОтчета.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	    ЭлементОтбора.ЛевоеЗначение = НовоеПолеОтбора;
	    ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	...
	КонецЕсли;
//}}20200428 ГлазуновДВ
//{{20200525 ГлазуновДВ Добавлено для автоматической подстановки отбора по полю "Заказ (об)"
	//НовоеПолеОтбора = Новый ПолеКомпоновкиДанных("АналитикаОборотов.Заказ");
	//Найден = Ложь;
	//Для Каждого ЭлементОтбора из НастройкиОтчета.Отбор.Элементы Цикл
	//	Если типЗНЧ(ЭлементОтбора) <> Тип("ЭлементОтбораКомпоновкиДанных") Тогда
	//		Продолжить;
	...
	//    ЭлементОтбора.Использование = Истина;
	//    //ЭлементОтбора.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный;
	//    ЭлементОтбора.ПравоеЗначение = Документы.ЗаказКлиента.ПустаяСсылка();
	//Иначе
	//    ЭлементОтбора = НастройкиОтчета.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	//    ЭлементОтбора.ЛевоеЗначение = НовоеПолеОтбора;
	//    ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
...
//Настройка структуры
	НастройкиОтчета.Структура[0].Структура[0].Использование = Истина;
//ИзменениеПериода
	НовоеПолеДанных = Новый ПараметрКомпоновкиДанных("Период");
	Для Каждого ЭлементДанных из НастройкиОтчета.ПараметрыДанных.Элементы Цикл
		Если типЗНЧ(ЭлементДанных) <> Тип("ЗначениеПараметраНастроекКомпоновкиДанных") Тогда
			Продолжить;
		КонецЕсли;
		Если ЭлементДанных.Параметр = НовоеПолеДанных Тогда
		    ЭлементДанных.Использование = Ложь;
			//ЭлементДанных.Значение.Вариант = ""
		    ЭлементДанных.Значение.ДатаНачала = Дата('00010101');
		    ЭлементДанных.Значение.ДатаОкончания = Дата('00010101');
		КонецЕсли;
	КонецЦикла;
//}}20200525 ГлазуновДВ
    ...
    //++Шерстюк Ю.Ю. 18.06.2021 обновление с 2.4.9.82
...
Функция ТекстЗапросаСебестоимостьТоваровОборотыПоЗаписям()
	...
	|	СебестоимостьТоваровЗаписи.Партия КАК Партия,
	|	АналитикаНоменклатуры.Номенклатура                     КАК Номенклатура,
	|	АналитикаНоменклатуры.Номенклатура.ЕдиницаИзмерения    КАК ЕдиницаХранения,
	|	АналитикаНоменклатуры.Номенклатура.ЕдиницаДляОтчетов   КАК ЕдиницаДляОтчетов,
	|	АналитикаНоменклатуры.Номенклатура.КоэффициентЕдиницыДляОтчетов   КАК КоэффициентЕдиницыДляОтчетов,
	...
	|	СебестоимостьТоваровЗаписи.Партия.*,
