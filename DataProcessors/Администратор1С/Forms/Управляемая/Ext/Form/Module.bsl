...
Процедура ПриСозданииНаСервере(Отказ)
	...
	НовоеПоле.УстановитьДействие("Открытие", "ТСсылкиОткрытие");
...
Процедура ОбъектБДСформироватьЗаголовокЭлементаФормы(ТипОбъектаБД, Элемент, ПолноеСтроковоеИмяТипа = "", Предопределенный = Ложь)
	...
	Иначе
		// ТипОбъектаБД = "Справочник" ИЛИ ТипОбъектаБД = "БизнесПроцесс" ИЛИ ТипОбъектаБД = "Задача" ИЛИ ТипОбъектаБД = "ПланСчетов" ИЛИ ТипОбъектаБД = "ПланОбмена" ИЛИ ТипОбъектаБД = "ПланВидовХарактеристик" ИЛИ ТипОбъектаБД = "ПланВидовРасчета".
...
Процедура ОбъектБДОчистка(Элемент, СтандартнаяОбработка = Истина) //*yuraos, 10.09.2015	//Процедура ОбъектБДОчистка(Элемент) //-yuraos, 10.09.2015
	...
	// Вкладка "Дополнительно" - "Документ (Удаление ГС)".
	ДокументУдалениеГСОчиститьПараметры();
...
Процедура ТСсылкиОткрытие(Элемент, СтандартнаяОбработка)
	Перем Меню, ПараметрыОповещенияОЗавершении, ТекущиеДанные, Выбор, Оповещение;
			
	Если (ТипЗнч(Элемент) = Тип("ПолеФормы") И Элемент.Имя = "Данные") 
		ИЛИ (ТипЗнч(Элемент) = Тип("ТаблицаФормы") И Элемент.ТекущийЭлемент.Имя = "Данные") Тогда
		
		СтандартнаяОбработка = Ложь;
		
		Если ТипЗнч(Элемент) = Тип("ПолеФормы") Тогда
			ТекущиеДанные = Элемент.Родитель.ТекущиеДанные.Данные;
		Иначе
			ТекущиеДанные = Элемент.ТекущиеДанные.Данные;
		КонецЕсли;
		
		ПараметрыОповещенияОЗавершении = Новый Структура("Элемент, ТекущиеДанные, СтандартнаяОбработка", Элемент, ТекущиеДанные, СтандартнаяОбработка);
		
		Меню = Новый СписокЗначений;
		Меню.Добавить(0, "Открыть в форме объекта: " + ТекущиеДанные, , БиблиотекаКартинок.Лупа);
		Если ЭтоСсылка(ТипЗнч(ТекущиеДанные)) Тогда
			Меню.Добавить(1, "Открыть в форме Администратор: " + ТекущиеДанные, , БиблиотекаКартинок.Изменить);
		Иначе
			ТСсылкиОткрытиеЗавершение(0, ПараметрыОповещенияОЗавершении);
			Возврат;
		КонецЕсли;
		...
		Если ИспользоватьРежимМодальности() Тогда
			...
			ТСсылкиОткрытиеЗавершение(Выбор, ПараметрыОповещенияОЗавершении);
		Иначе
			// Стандартно в немодальном режиме (8.3) с обработкой результата.
			Оповещение = Вычислить("Новый ОписаниеОповещения(""ТСсылкиОткрытиеЗавершение"", ЭтаФорма, ПараметрыОповещенияОЗавершении)");
...
&НаКлиенте
Процедура ТСсылкиОткрытиеЗавершение(Выбор, ПараметрыОповещения) Экспорт
	Перем ТекущееЗначение, РежимОткрытияФормы;
	...
	Если Выбор = Неопределено Тогда
		Возврат;
	ИначеЕсли ТипЗнч(Выбор) = Тип("ЭлементСпискаЗначений") И Выбор.Значение = 1 Тогда
		НовоеОкноАдминистатора1С(Истина, ТекущиеДанные);
	Иначе
		РежимОткрытияФормы = ОткрытьФормуОбъектаМодальноИлиНемодальноВРежимеБлокировкиВладельца(ТекущиеДанные, ПараметрыОповещения.СтандартнаяОбработка);
		Если РежимОткрытияФормы = "Стандартно" Тогда
			Попытка
				// По-возможности используем асинхронный метод.
				Выполнить("ПоказатьЗначение(, ТекущиеДанные);");
			Исключение
				ОткрытьЗначение(ТекущиеДанные);
			КонецПопытки;
		КонецЕсли;
	КонецЕсли;
...
Функция ЭтоСсылка(Тип)
	Возврат ОбъектОбработка().ЭтоСсылка(Тип);
КонецФункции
...
&НаКлиентеНаСервереБезКонтекста
Функция ПреобразоватьСекундыВДату(Секунды)
	Перем Результат;
	...
	Если НЕ (Часов > 23 ИЛИ Минут > 59 ИЛИ Секунд > 59) Тогда
...
Функция ПреобразоватьСекундыВоВремя(Секунды)
	...
	Часов = Цел(Секунды / 3600);
	Минут = Цел((Секунды - Часов * 3600) / 60);
	Секунд = Цел(Секунды - ((Минут * 60) + (Часов * 3600)));
	
	Возврат "" + ?(Часов = 0, "00", Формат(Часов, "ЧЦ=2; ЧВН=")) + ":" + ?(Минут = 0, "00", Формат(Минут, "ЧЦ=2; ЧВН=")) + ":" + ?(Секунд = 0, "00", Формат(Секунд, "ЧЦ=2; ЧВН="));
...
////////////////////////////////////////////////////////////////////////////////
// ДОКУМЕНТЫ: Удаление документов (ГРУБАЯ СИЛА).
//

&НаКлиенте
Процедура ПолучитьНачальныеПараметрыДляУдаленияДокументовГС(Команда)
	
	ДокументУдалениеГСОчиститьПараметры();
	
	ЭтаФорма.ДокументУдалениеГСПериод.ДатаНачала = НачалоГода(ТекущаяДата());
	ЭтаФорма.ДокументУдалениеГСПериод.ДатаОкончания = ТекущаяДата();
	
	ЭтаФорма.ДокументУдалениеГСДействие = "ПометитьНаУдаление";
	
	УстановитьЗначениеПараметраНаВкладке("ДокументУдалениеГСОрганизации"	, "Организация"		, "Организации"		, Объект.ЕстьСправочникОрганизации);
	
	ПолучитьНачальныеПараметрыДляУдалениеДокументовГСНаСервере();
...
//
&НаСервере
Процедура ПолучитьНачальныеПараметрыДляУдалениеДокументовГСНаСервере()
	
	ТаблицаСписокВидовДокументов = РеквизитФормыВЗначение("ДокументУдалениеГСТСписокВидов");
	ТаблицаСписокВидовДокументов = ОбъектОбработка().ДокументУдалениеГСЗаполнитьСписокВидовДокументов(ТаблицаСписокВидовДокументов, ЭтаФорма.ДокументУдалениеОрганизации);
	ЗначениеВРеквизитФормы(ТаблицаСписокВидовДокументов, "ДокументУдалениеГСТСписокВидов");
	
КонецПроцедуры

&НаКлиенте
Процедура ДокументУдалениеГСОчиститьПараметры()
	
	ЭтаФорма.ДокументУдалениеГСКомментарий = "";
	
	ЭтаФорма.ДокументУдалениеГСПериод.ДатаНачала = Дата("00010101");
	ЭтаФорма.ДокументУдалениеГСПериод.ДатаОкончания = Дата("00010101");
	
	ЭтаФорма.ДокументУдалениеГСДействие = "ПометитьНаУдаление";
	
	ЭтаФорма.ДокументУдалениеГСОрганизации = Новый СписокЗначений;
	
КонецПроцедуры

&НаКлиенте
Процедура ДокументУдалениеГСПействиеПриИзменении(Элемент)
	Если ЭтаФорма.ДокументУдалениеГСДействие = "Удалить" Тогда
		ПредупреждениеСообщение(, "НЕПОСРЕДСТВЕННОЕ УДАЛЕНИЕ ОБЪЕКТОВ
		|БЕЗ ВОЗМОЖОСТИ ВОССТАНОВЛЕНИЯ !!!
		|
		|ДЕЙСТВИЕ НЕОБРАТИМО !!!");
	Иначе
		ПредупреждениеСообщение(, "ПОМЕТКА УДАЛЕНИЯ ДОКУМЕНТОВ
		|
		|Для фактического удаления документов необходимо воспользоваться обработкой
		|""Удаление помеченных объектов"".");
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ДокументУдалениеГСПроизвестиУдалениеДокументовГС(Команда)
	Перем ТекстВопроса, Ответ, Оповещение, Параметр;
	
	Если ЭтаФорма.ДокументУдалениеГСОрганизации.Количество() = 0 Тогда
		ПредупреждениеСообщение(, "Не выбрана(ы) организация(и).");
		Возврат;
	КонецЕсли;
	...
	Если ЭтаФорма.ДокументУдалениеГСДействие = "Удалить" Тогда
		...
		|
		|Произвести удаление документов для выбранных организаций?
		|";
	Иначе
		ТекстВопроса = "
		|ПОМЕТКА УДАЛЕНИЯ ДОКУМЕНТОВ.
		|
		|Произвести удаление документов для выбранных организаций?
		|";
	КонецЕсли;
	...
	Если ИспользоватьРежимМодальности() Тогда
		// Стандартно в модальном режиме (8.2/8.3) с обработкой результата.
		Ответ = Вопрос(ТекстВопроса, РежимДиалогаВопрос.ДаНет, 120, КодВозвратаДиалога.Нет, ЗаголовокВопроса, КодВозвратаДиалога.Нет);
		ДокументУдалениеГСПроизвестиУдалениеДокументовЗавершение(Ответ, Параметр);
	Иначе
		// Стандартно в немодальном режиме (8.3) с обработкой результата.
		//+yuraos, 10.09.2015 - для обратной совместимости лучше использовать атрибут "ЭтаФорма", "ЭтотОбъект" появляется в режиме совместимости "Версия 8.3.3" и выше.
		Оповещение = Вычислить("Новый ОписаниеОповещения(""ДокументУдалениеГСПроизвестиУдалениеДокументовЗавершение"", ЭтаФормаЭтотОбъект(), Параметр)");
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет, 120, КодВозвратаДиалога.Нет, ЗаголовокВопроса, КодВозвратаДиалога.Нет);
	КонецЕсли;
...
&НаКлиенте
Процедура ДокументУдалениеГСПроизвестиУдалениеДокументовЗавершение(Ответ, Параметр) Экспорт
	Перем ЗначениеОтвета, Сообщение;
	Перем ВремяНачала, ВремяОкончания, ВремяВСекундах;
	...
	Состояние("Подождите. Производится удаление документов ...");
	ДокументУдалениеГСПроизвестиУдалениеДокументовНаСервере();
	
	ВремяОкончания = ВремяВМиллисекундах();
	ВремяВСекундах = (ВремяОкончания - ВремяНачала)/1000;
	
	Сообщение = "Затрачено " + ПреобразоватьСекундыВоВремя(ВремяВСекундах);
	ЭтаФорма.ДокументУдалениеГСКомментарий = ЭтаФорма.ДокументУдалениеГСКомментарий + Сообщение;
	
КонецПроцедуры

&НаСервере
Процедура ДокументУдалениеГСПроизвестиУдалениеДокументовНаСервере()
	
	ОбъектОбработка().ДокументУдалениеГСПроизвестиУдалениеДокументовНаСервере(ЭтаФорма.ДокументУдалениеГСДействие, ЭтаФорма.ДокументУдалениеГСТСписокВидов, ЭтаФорма.ДокументУдалениеГСПериод, ЭтаФорма.ДокументУдалениеГСОрганизации, ЭтаФорма.ДокументУдалениеГСКомментарий);
	
КонецПроцедуры

&НаКлиенте
Процедура ДокументУдалениеГСУстановитьФлажки(Команда)
	ПолучитьНачальныеПараметрыДляУдалениеДокументовГСНаСервере();
КонецПроцедуры

&НаКлиенте
Процедура ДокументУдалениеГССнятьФлажки(Команда)
	Перем СтрокаСписка;
	
	Состояние("Подождите. Производится снятие пометки данных ...");
	Для Каждого СтрокаСписка ИЗ ЭтаФорма.ДокументУдалениеГСТСписокВидов Цикл
...
Функция ВерсияПриложенияМеньше8310()
	...
	ВерсияПриложенияБезНомераСборки = СтрЗаменить(Объект.ВерсияПриложенияБезНомераСборки, ".", "");
	ВерсияПриложенияБезНомераСборки = Число(ВерсияПриложенияБезНомераСборки);
	Возврат ВерсияПриложенияБезНомераСборки <= 839;
КонецФункции
...
Процедура ОбъектБДОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Иначе
		...
		Если Объект.ОбъектБД <> Неопределено И (ТипЗнч(ВыбранноеЗначение) <> ТипЗнч(Объект.ОбъектБД) ИЛИ (ТипЗнч(ВыбранноеЗначение) = ТипЗнч(Объект.ОбъектБД) И НЕ ВыбранноеЗначение = Объект.ОбъектБД)) Тогда
			ОбъектБДОчистка(Элемент, Истина);
		КонецЕсли;
		ВызватьОбъектБДПриИзменении(ВыбранноеЗначение, Элемент);
	КонецЕсли;
...
Процедура ОбъектБДОчисткаМеню(Элемент, СтандартнаяОбработка)
	...
	// Для широкого списка на форме.
	СтрокаПредставление = "<Очистить значение Объекта БД>";
	СтрокаПредставление = ДополнитьСтроку(СтрокаПредставление, 100, " ", "Справа");
	Меню.Вставить(0, Неопределено	, СтрокаПредставление			, , БиблиотекаКартинок.Удалить);
	//
...
&НаКлиенте
Процедура РегистрыСведенийПараметры(Команда)
	ОчиститьСообщения();
	Состояние("Формирование списков регистров ...");
	РегистрыСведенийПараметрыНаСервере();
КонецПроцедуры

&НаСервере
Процедура РегистрыСведенийПараметрыНаСервере()
	Перем ТипРегистра, ИмяРегистра, мдРегистра, КоличествоЗаписей, КоличествоРегистраторов, НоваяСтрока;
	
	ТипРегистра = "РегистрСведений";
	ЭтаФорма.РегистрыСведенийНезависимые.Очистить();
	ЭтаФорма.РегистрыСведенийПодчиненныеРегистратору.Очистить();
	
	Для Каждого мдРегистра ИЗ МЕтаданные.РегистрыСведений Цикл
		ИмяРегистра = мдРегистра.Имя;
		КоличествоЗаписей = ПолучитьКоличествоЗаписейРегистра(ТипРегистра, ИмяРегистра);
		Если мдРегистра.РежимЗаписи = Метаданные.СвойстваОбъектов.РежимЗаписиРегистра.Независимый Тогда
			НоваяСтрока = ЭтаФорма.РегистрыСведенийНезависимые.Добавить();
			НоваяСтрока.НомерСтроки = ЭтаФорма.РегистрыСведенийНезависимые.Количество();
			НоваяСтрока.ИмяРегистра = мдРегистра.Имя;
			НоваяСтрока.ПериодичностьРегистраСведений = мдРегистра.ПериодичностьРегистраСведений;
			НоваяСтрока.СинонимРегистра = мдРегистра.Синоним;
			НоваяСтрока.Регистраторы = Неопределено;
			НоваяСтрока.РегистраторыСтрока = НоваяСтрока.Регистраторы;	// Метаданные.
			НоваяСтрока.КоличествоРегистраторов = 0;
			НоваяСтрока.КоличествоЗаписей = КоличествоЗаписей;
		Иначе
			НоваяСтрока = ЭтаФорма.РегистрыСведенийПодчиненныеРегистратору.Добавить();
			НоваяСтрока.НомерСтроки = ЭтаФорма.РегистрыСведенийПодчиненныеРегистратору.Количество();
			НоваяСтрока.ИмяРегистра = мдРегистра.Имя;
			НоваяСтрока.ПериодичностьРегистраСведений = мдРегистра.ПериодичностьРегистраСведений;
			НоваяСтрока.СинонимРегистра = мдРегистра.Синоним;
			НоваяСтрока.Регистраторы = ПолучитьСписокРегистраторовРегистра(мдРегистра);	// ОписаниеТипов.
			НоваяСтрока.РегистраторыСтрока = НоваяСтрока.Регистраторы;	// Метаданные.
			НоваяСтрока.КоличествоРегистраторов = ПолучитьКоличествоРегистраторовРегистра(ТипРегистра, ИмяРегистра);
			НоваяСтрока.КоличествоЗаписей = КоличествоЗаписей;
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Функция ПолучитьСписокРегистраторовРегистра(мдРегистра)
	Перем Регистраторы;
	
	Регистраторы = мдРегистра.СтандартныеРеквизиты.Регистратор.Тип;
	
	Возврат Регистраторы;
КонецФункции

&НаСервере
Функция ПолучитьКоличествоРегистраторовРегистра(ТипРегистра, ИмяРегистра)
	Перем Запрос, Результат, Выборка, КоличествоРегистраторов;
	
	Запрос = Новый Запрос;
	Запрос.Текст = "
	|ВЫБРАТЬ
	|КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Регистр.Регистратор) КАК КоличествоРегистраторов
	|ИЗ
	|" + ТипРегистра + "." + ИмяРегистра + " КАК Регистр";
	...
	Выборка = Результат.Выбрать();
	Выборка.Следующий();
	КоличествоРегистраторов = Выборка.КоличествоРегистраторов;
	
	Возврат КоличествоРегистраторов;

КонецФункции

&НаСервере
Функция ПолучитьКоличествоЗаписейРегистра(ТипРегистра, ИмяРегистра)
	Перем Запрос, Результат, Выборка, КоличествоЗаписей;
...
&НаКлиенте
Процедура ОткрытьРегистрСведенийНезависимый(Команда)
	Перем ИмяРегистра;
	
	Если НЕ ЭтаФорма.ТекущийЭлемент = Элементы.РегистрыСведенийНезависимые Тогда
		ПредупреждениеСообщение(, "Выберите независимый регистр сведений.");
		Возврат;
	КонецЕсли;
	
	ИмяРегистра = ЭтаФорма.ТекущийЭлемент.ТекущиеДанные.ИмяРегистра;
	ОткрытьФормуРегистра("РегистрСведений", ИмяРегистра, "ФормаСписка");
	
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьРегистрСведенийПодчиненныйРегистратору(Команда)
	Перем ИмяРегистра;
	
	Если НЕ ЭтаФорма.ТекущийЭлемент = Элементы.РегистрыСведенийПодчиненныеРегистратору Тогда
		ПредупреждениеСообщение(, "Выберите регистр сведений подчиненный регистратору.");
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ОткрытьФормуРегистра(ТипРегистра, ИмяРегистра, ФормаРегистра = "ФормаСписка");
	Попытка
		ОткрытьФорму("" + ТипРегистра + "." + ИмяРегистра + "." + ФормаРегистра);
	Исключение
		Сообщить(ОписаниеОшибки());
	КонецПопытки;
КонецПроцедуры
