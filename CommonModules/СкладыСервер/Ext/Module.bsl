...
//
Функция РИН1_ПереоформитьРасходныеОрдера(Параметры) Экспорт
...
//{{20200423 ГлазуновДВ Добавили возможность исключать в ордере товар, который находится в блокировке по галочке в Документе ПересчетТоваров
//	ТаблицаНоменклатурыДляЗапросаДО = ОбщегоНазначенияУТ.ДанныеДерева(ДеревоТоварыКОтгрузке, 6);	

	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТаблицаНоменклатурыДляЗапросаДО.Номенклатура КАК Номенклатура
		|ПОМЕСТИТЬ ТабНоменклатура
		|ИЗ
		|	&ТаблицаНоменклатурыДляЗапросаДО КАК ТаблицаНоменклатурыДляЗапросаДО
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ПересчетТоваровТовары.Номенклатура КАК Номенклатура,
		|	ПересчетТоваровТовары.Ссылка.Рин1_БлокироватьОтобранное КАК Рин1_БлокироватьОтобранное
		|ПОМЕСТИТЬ ТабНоменклатура2
		|ИЗ
		...
		|ИЗ
		|	ТабНоменклатура2 КАК ТабНоменклатура2
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТабНоменклатура КАК ТабНоменклатура
		|		ПО (ТабНоменклатура.Номенклатура = ТабНоменклатура2.Номенклатура)
		|ГДЕ
		|	ТабНоменклатура2.Рин1_БлокироватьОтобранное = &Рин1_БлокироватьОтобранное";
	
	Запрос.УстановитьПараметр("ТаблицаНоменклатурыДляЗапросаДО", ТаблицаНоменклатурыДляЗапроса);
	Запрос.УстановитьПараметр("Рин1_БлокироватьОтобранное", Истина);
	
	РезультатЗапроса = Запрос.Выполнить();
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
	//ТаблицаНоменклатурыДляЗапроса = ТаблицаНоменклатурыДляЗапросаДО.Скопировать();
	Если ВыборкаДетальныеЗаписи.Количество() > 0 Тогда
		//ТаблицаНоменклатурыДляЗапроса.Очистить();
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			// Вставить обработку выборки ВыборкаДетальныеЗаписи
		    Индекс = ТаблицаНоменклатурыДляЗапроса.Количество() - 1; 
		    Пока Не Индекс < 0 Цикл 
	    		Если ТаблицаНоменклатурыДляЗапроса[Индекс].Номенклатура = ВыборкаДетальныеЗаписи.Номенклатура Тогда 
	    			ТаблицаНоменклатурыДляЗапроса.Удалить(Индекс); 
	    			Прервать; 
				КонецЕсли;
				Индекс = Индекс -1;
			КонецЦикла;
