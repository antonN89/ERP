...
//}}202008031 ГлазуновДВ
	...
	УстановитьНадписьДокументыОбОтгрузки()
	// - [Rineco], [Киселев А.Н.] [13.08.2021]
КонецПроцедуры
...
#Область Задача_21791

// + [Rineco], [Киселев А.Н.] [10.11.2021] 
// Задача: [№ 21791], [#ОбработкаНадписи]
&НаСервере
Процедура УстановитьНадписьДокументыОбОтгрузки()
	...
	Если Объект.ОтредактированыДокументыОтгрузки Тогда
		...
		Если Не ЗначениеЗаполнено(НадписьДокументыОбОтгрузке) Тогда
			ЭтаФорма.НадписьДокументыОбОтгрузке = НСтр("ru = '№ п/п  №  от  '");
		Иначе
			ЭтаФорма.НадписьДокументыОбОтгрузке = НадписьДокументыОбОтгрузке;
		КонецЕсли;
	Иначе
		ЭтаФорма.НадписьДокументыОбОтгрузке = НСтр("ru = '<Авто>'");
	КонецЕсли;
	
КонецПроцедуры

// + [Rineco], [Киселев А.Н.] [10.11.2021] 
// Задача: [№ 21791], [#ДокументыОбОтгрузке]
&НаКлиенте
Процедура Рин1_НадписьДокументыОбОтгрузкеНажатиеВместо(Элемент, СтандартнаяОбработка)
	...
	СтруктураПараметров.Вставить("ВидСчетаФактуры",                  Неопределено);	
	СтруктураПараметров.Вставить("АдресТаблицДокумента",             ПоместитьТаблицыДокументаВХранилище());
	СтруктураПараметров.Вставить("ВключитьВозможностьРедактировать", ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Объект.Ссылка));
...
Функция ПоместитьТаблицыДокументаВХранилище()
	...
	ТаблицыДокумента.Вставить("ТаблицаДокументовОснований",  Новый Массив);
...
Процедура Рин1_ОбработкаВыбораПосле(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ИсточникВыбора.ИмяФормы = "ОбщаяФорма.ДокументыОбОтгрузкеСчетаФактуры" Тогда
		Модифицированность	= Истина;
