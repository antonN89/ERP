...
// Задача: [№ 21791], [# Выводим строку 5а в зав-от от заполнения нового реквизита]
&Вместо("ЗаполнитьРеквизитыШапкиСчетФактура")
Процедура Рин1_ЗаполнитьРеквизитыШапкиСчетФактура(ДанныеПечати, ДанныеОснований, СведенияОПоставщике, ДанныеКонтрагентов, ТабличныйДокумент, ОпцииПечатиСчетаФактуры)
	...
	Если НЕ ДанныеПечати.КорректировочныйСчетФактура Тогда
		...
		// + [Rineco], [Киселев А.Н.] [10.11.2021] 
		// Задача: [№ 21791], [#Вывод по документы отгрузки]
		...
		Если ЗначениеЗаполнено(ДанныеПечати.Ссылка.ДокументОснование) И ТипЗнч(ДанныеПечати.Ссылка.ДокументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг") И ЗначениеЗаполнено(ДанныеПечати.Ссылка.ДокументОснование.ДокументыОбОтгрузке)  Тогда
			
			
			ШаблонПодстрокиДокументаОбОтгрузке = "№ п/п %1 №%2 от %3 г.";
			ДокументыОбОтгрузке = "";
			Для Каждого ОтгрузочныйДокумент Из ДанныеПечати.Ссылка.ДокументОснование.ДокументыОбОтгрузке Цикл
				...
				КонецЕсли;
			КонецЦикла;
			ПоДокументуОтгрузки = ДокументыОбОтгрузке;
		Иначе
			ПоДокументуОтгрузки =
			?(ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеПечати, "ПредставлениеСтроки5а")
			И ЗначениеЗаполнено(ДанныеПечати.ПредставлениеСтроки5а),
