...
Процедура УстановитьВидимостьЗаголовки()
	...
	// + [Rineco], [Киселев А.Н.] [12.10.2021] 
	// Задача: [№ 19719], [#ВидимостьПолей]
	СтруктураПараметров.Вставить("ОтображатьХарактеристику",					Ложь);
	СтруктураПараметров.Вставить("ОтображатьСерию",								Ложь);
	СтруктураПараметров.Вставить("ОтображатьНазначение",						Ложь);
	// - [Rineco], [Киселев А.Н.] [12.10.2021]
	...
	// + [Rineco], [Киселев А.Н.] [12.10.2021] 
	// Задача: [№ 19719], [#Видимость заголовков]
	ЗначениеСвойства = СтруктураПараметров.ОтображатьХарактеристику;
	Элементы.ПолноеДеревоСебестоимостиХарактеристика.Видимость = ЗначениеСвойства;
	
	ЗначениеСвойства = СтруктураПараметров.ОтображатьСерию;
	Элементы.ПолноеДеревоСебестоимостиСерия.Видимость = ЗначениеСвойства;
	
	ЗначениеСвойства = СтруктураПараметров.ОтображатьНазначение;
	Элементы.ПолноеДеревоСебестоимостиНазначение.Видимость = ЗначениеСвойства;
	// - [Rineco], [Киселев А.Н.] [12.10.2021]
...
Функция ФоновоеЗаданиеЗапустить()
	...
	// Задача: [№ 19719], [#ДеревоСебестоимости]
	...
	// Стало:
	РезультатФоновогоЗадания = ДлительныеОперации.ВыполнитьВФоне(
		"СтруктураСебестоимости.РИНЭКО_ПостроитьДеревоСебестоимостиВФоне",
		ПараметрыФормированияОтчета,
		ПараметрыЗапуска);
	// - [Rineco], [Киселев А.Н.] [11.10.2021]
...
Процедура ПолноеДеревоСебестоимостиПередРазворачиваниемСервер(ИдентификаторСтроки, СПодчиненными = Ложь)
	...
	// Стало:
	СтруктураСебестоимости.РИНЭКО_ДеревоСебестоимостиПередРазворачиванием(ПараметрыДерева, ПараметрыУзла, ОписаниеПродукции);
	// - [Rineco], [Киселев А.Н.] [12.10.2021]
