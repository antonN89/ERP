...
// + [Rineco], [Киселев А.] [28.11.2021] [Log_Diff][№ 11841], [#Заявка на расходование ДС] 
Процедура Рин1_ОбработкаПроведения(Источник, Отказ) Экспорт
	...
	Если Источник.Статус <> Перечисления.СтатусыЗаявокНаРасходованиеДенежныхСредств.НеСогласована или Источник.Статус <> Перечисления.СтатусыЗаявокНаРасходованиеДенежныхСредств.Отклонена Тогда 
		...
		// Задача: [№ 11841], [#Заявка на расходование ДС]
		...
		// Стало:
		Если Источник.ФормаОплатыБезналичная и (Не ЗначениеЗаполнено(Источник.БанковскийСчетКонтрагента) и Не ЗначениеЗаполнено(Источник.БанковскийСчетПолучатель) И Не Источник.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗарплаты) Тогда 
		// - [Rineco], [Киселев А.] [26.11.2021]
