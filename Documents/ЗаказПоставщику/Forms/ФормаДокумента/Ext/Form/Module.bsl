...
// + [Rineco], [Киселев А.Н.] [02.11.2021] 
// Задача: [№ 21645], [# Проверить погашенное количество]
&НаСервере
Процедура Рин1_ПередЗаписьюНаСервереПосле(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	ПроверитьКоличествоСервер();
КонецПроцедуры
...
// Задача: [№ 21645], [# Проверить погашенное количество]
&НаСервере
Процедура ПроверитьКоличествоСервер()
	
	ПараметрыЗапроса = Новый Структура;
	ТабличнаяЧастьЗП = Объект.Товары.Выгрузить(); 
	ТабличнаяЧастьЗП.Колонки.Добавить("Ссылка",Новый ОписаниеТипов("ДокументСсылка.ЗаказПоставщику"));
	...
	Для Каждого ЭлементТаблицы Из ТаблицаДанных Цикл 
		...
		Сообщить("Номенклатура " + ЭлементТаблицы.НоменклатураПриход + " с кодом строки: " + ЭлементТаблицы.КодСтрокиПриход + " погашена в количестве " + ЭлементТаблицы.КОформлениюРасход);
...
Функция  ТекстЗапросаПогашенныеПозиции()
	...
	Возврат "ВЫБРАТЬ
	        ...
	        |	ТаблицаТоваров.СтрокаУдалена КАК СтрокаУдалена
	        ...
	        |	ВТ_ТоварыЗП.СтрокаУдалена КАК СтрокаУдалена
	        ...
	        |		ПО ВТ_ТоварыЗП.Ссылка = ЗаказыПоставщикам.ЗаказПоставщику
	        ...
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ
	        |	ВТ_ТоварыЗП.Ссылка КАК ЗаказРасход,
	        |	ВТ_ТоварыЗП.КоличествоУпаковок КАК КоличествоУпаковокРасход,
	        |	ЗаказыПоставщикам.Заказано КАК ЗаказаноРасход,
	        |	ЗаказыПоставщикам.КОформлению КАК КОформлениюРасход,
	        |	ЗаказыПоставщикам.КодСтроки КАК КодСтрокиРасход,
	        |	ЗаказыПоставщикам.Номенклатура КАК НоменклатураРасход,
	        |	ЗаказыПоставщикам.Склад КАК СкладРасход,
	        |	ЗаказыПоставщикам.Характеристика КАК ХарактеристикаРасход,
	        |	ВТ_ТоварыЗП.СтрокаУдалена КАК СтрокаУдалена
	        |ПОМЕСТИТЬ ВТ_ЗаказРасход
	        |ИЗ
	        |	ВТ_ТоварыЗП КАК ВТ_ТоварыЗП
	        |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗаказыПоставщикам КАК ЗаказыПоставщикам
	        |		ПО ВТ_ТоварыЗП.Номенклатура = ЗаказыПоставщикам.Номенклатура
	        |			И ВТ_ТоварыЗП.Характеристика = ЗаказыПоставщикам.Характеристика
	        |			И (ЗаказыПоставщикам.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход))
	        |			И ВТ_ТоварыЗП.Ссылка = ЗаказыПоставщикам.ЗаказПоставщику
	        |;
	        |
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ
	        ...
	        |	ВТ_ЗаказРасход.ЗаказРасход КАК ЗаказРасход,
	        |	ВТ_ЗаказРасход.КоличествоУпаковокРасход КАК КоличествоУпаковокРасход,
	        |	ВТ_ЗаказРасход.ЗаказаноРасход КАК ЗаказаноРасход,
	        |	ВТ_ЗаказРасход.КОформлениюРасход КАК КОформлениюРасход,
	        |	ВТ_ЗаказРасход.КодСтрокиРасход КАК КодСтрокиРасход,
	        |	ВТ_ЗаказРасход.НоменклатураРасход КАК НоменклатураРасход,
	        |	ВТ_ЗаказРасход.СкладРасход КАК СкладРасход,
	        |	ВТ_ЗаказРасход.ХарактеристикаРасход КАК ХарактеристикаРасход,
	        |	ВТ_ЗаказПриход.КоличествоПриход - ЕСТЬNULL(ВТ_ЗаказРасход.КОформлениюРасход, 0) КАК Разница,
	        |	ВТ_ЗаказПриход.СтрокаУдалена КАК СтрокаУдалена,
	        |	ВТ_ЗаказРасход.СтрокаУдалена КАК СтрокаУдалена1
	        |ПОМЕСТИТЬ ВТ_ИтоговаяТаблица
	        |ИЗ
	        |	ВТ_ЗаказПриход КАК ВТ_ЗаказПриход
	        |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ЗаказРасход КАК ВТ_ЗаказРасход
	        |		ПО ВТ_ЗаказПриход.ЗаказПриход = ВТ_ЗаказРасход.ЗаказРасход
	        |			И ВТ_ЗаказПриход.НоменклатураПриход = ВТ_ЗаказРасход.НоменклатураРасход
	        |			И ВТ_ЗаказПриход.ХарактеристикаПриход = ВТ_ЗаказРасход.ХарактеристикаРасход
	        |			И ВТ_ЗаказПриход.СкладПриход = ВТ_ЗаказРасход.СкладРасход
	        |			И ВТ_ЗаказПриход.КодСтрокиПриход = ВТ_ЗаказРасход.КодСтрокиРасход
	        |;
	        |
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ РАЗЛИЧНЫЕ
	        |	ВТ_ИтоговаяТаблица.ЗаказаноПриход КАК ЗаказаноПриход,
	        |	СУММА(ВТ_ИтоговаяТаблица.ЗаказаноРасход) КАК ЗаказаноРасход,
	        |	ВТ_ИтоговаяТаблица.ЗаказПриход КАК ЗаказПриход,
	        |	ВТ_ИтоговаяТаблица.ЗаказРасход КАК ЗаказРасход,
	        |	ВТ_ИтоговаяТаблица.КодСтрокиПриход КАК КодСтрокиПриход,
	        |	ВТ_ИтоговаяТаблица.КодСтрокиРасход КАК КодСтрокиРасход,
	        |	ВТ_ИтоговаяТаблица.КоличествоПриход КАК КоличествоПриход,
	        |	ВТ_ИтоговаяТаблица.КоличествоУпаковокРасход КАК КоличествоУпаковокРасход,
	        |	ВТ_ИтоговаяТаблица.КОформлениюПриход КАК КОформлениюПриход,
	        |	СУММА(ВТ_ИтоговаяТаблица.КОформлениюРасход) КАК КОформлениюРасход,
	        |	ВТ_ИтоговаяТаблица.НоменклатураПриход КАК НоменклатураПриход,
	        |	ВТ_ИтоговаяТаблица.НоменклатураРасход КАК НоменклатураРасход,
	        |	СУММА(ВТ_ИтоговаяТаблица.Разница) КАК Разница,
	        |	ВТ_ИтоговаяТаблица.СкладПриход КАК СкладПриход,
	        |	ВТ_ИтоговаяТаблица.СкладРасход КАК СкладРасход,
	        |	ВТ_ИтоговаяТаблица.ХарактеристикаПриход КАК ХарактеристикаПриход,
	        |	ВТ_ИтоговаяТаблица.ХарактеристикаРасход КАК ХарактеристикаРасход,
	        |	ВТ_ИтоговаяТаблица.СтрокаУдалена КАК СтрокаУдалена
	        |ИЗ
	        |	ВТ_ИтоговаяТаблица КАК ВТ_ИтоговаяТаблица
	        |ГДЕ
	        |	(ВТ_ИтоговаяТаблица.Разница <> 0
	        |			ИЛИ ВТ_ИтоговаяТаблица.СтрокаУдалена)
	        |	И НЕ ВТ_ИтоговаяТаблица.ЗаказРасход ЕСТЬ NULL
	        |
	        |СГРУППИРОВАТЬ ПО
	        |	ВТ_ИтоговаяТаблица.НоменклатураРасход,
	        |	ВТ_ИтоговаяТаблица.КоличествоПриход,
	        |	ВТ_ИтоговаяТаблица.СкладПриход,
	        |	ВТ_ИтоговаяТаблица.СкладРасход,
	        |	ВТ_ИтоговаяТаблица.ХарактеристикаПриход,
	        |	ВТ_ИтоговаяТаблица.ХарактеристикаРасход,
	        |	ВТ_ИтоговаяТаблица.СтрокаУдалена,
	        |	ВТ_ИтоговаяТаблица.ЗаказПриход,
	        |	ВТ_ИтоговаяТаблица.КоличествоУпаковокРасход,
	        |	ВТ_ИтоговаяТаблица.НоменклатураПриход,
	        |	ВТ_ИтоговаяТаблица.ЗаказаноПриход,
	        |	ВТ_ИтоговаяТаблица.КодСтрокиПриход,
	        |	ВТ_ИтоговаяТаблица.КодСтрокиРасход,
	        |	ВТ_ИтоговаяТаблица.КОформлениюПриход,
	        |	ВТ_ИтоговаяТаблица.ЗаказРасход";
