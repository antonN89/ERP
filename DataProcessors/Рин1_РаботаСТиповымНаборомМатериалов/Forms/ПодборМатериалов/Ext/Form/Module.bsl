...
<<<<<<< HEAD
			НоваяСтрока.СтатьяКалькуляции = ЭлементДанныхВторойУровень.ТиповойНаборМатериалов.СтатьяКалькуляции;
=======
...
<<<<<<< HEAD
		...
		Отбор.Вставить("Номенклатура",Элемент.Родитель.ТекущиеДанные.Номенклатура);
		НайденныеСтроки = Объект.ТаблицаВыбранных.НайтиСтроки(Отбор);
		Если ЗначениеЗаполнено(НайденныеСтроки) Тогда
			Для Каждого ЭлементСтроки Из НайденныеСтроки Цикл 
				ЗаполнитьЗначенияСвойств(ЭлементСтроки,Элемент.Родитель.ТекущиеДанные);
			КонецЦикла;
		Иначе 
...
<<<<<<< HEAD
	ТаблицаПрототип.Колонки.Добавить("СтатьяКалькуляции", Новый ОписаниеТипов("СправочникСсылка.СтатьиКалькуляции"),"СтатьяКалькуляции");
=======
>>>>>>> parent of 7be3d8f (Задача 00000 (в коммите много задач))
