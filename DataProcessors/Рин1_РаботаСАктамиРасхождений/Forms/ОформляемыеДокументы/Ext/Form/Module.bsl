...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВыводитьЛегенду = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("Обработка.Рин1_РаботаСАктамиРасхождений", "ВыводитьЛегенду", Ложь);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	//{{20200714 ГлазуновДВ	Включили оповещение по "Запись_ЗаказНаВнутреннееПотребление"
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		Если Область.Расшифровка.Данные = ПредопределенноеЗначение("Документ.ЗаказКлиента.ПустаяСсылка") Тогда
			ПараметрыФормы = Новый Структура;
			ПараметрыФормы.Вставить("АктОРасхождениях", Область.Расшифровка.Описание.АктОРасхождениях);
			ПараметрыФормы.Вставить("Реализация", Область.Расшифровка.Описание.ОснованиеАкта);
			
			ОткрытьФорму("Документ.ЗаказКлиента.Форма.ФормаДокумента", Новый Структура("Основание", ПараметрыФормы));
		ИначеЕсли
			Область.Расшифровка.Данные = ПредопределенноеЗначение("Документ.ЗаказНаВнутреннееПотребление.ПустаяСсылка") Тогда
			ПараметрыФормы = Новый Структура;
			ПараметрыФормы.Вставить("АктОРасхождениях", Область.Расшифровка.Описание.АктОРасхождениях);
			ПараметрыФормы.Вставить("Реализация", Область.Расшифровка.Описание.ОснованиеАкта);
			
			ОткрытьФорму("Документ.ЗаказНаВнутреннееПотребление.Форма.ФормаДокумента", Новый Структура("Основание", ПараметрыФормы));
		Иначе
...
Процедура СформироватьОтчет()
	...
	Макет = Обработки.Рин1_РаботаСАктамиРасхождений.ПолучитьМакет("ОформляемыеДокументы");
...
Процедура ЗаполнитьТаблицыСозданныхДокументов(РезультатЗапроса, ТипОснованияАктаОРасхождении, СоответствиеНаборовДанных)
	...
	Если ТипЗнч(АктОРасхождениях) = Тип("ДокументСсылка.АктОРасхожденияхПослеОтгрузки") Тогда
		...
		Если РасхожденияКлиентСервер.ТипОснованияРеализацияТоваровУслуг(ТипОснованияАктаОРасхождении) Тогда
			...
			Допоставки1.Загрузить(РезультатЗапроса[6].Выгрузить());
			Допоставки2.Загрузить(РезультатЗапроса[7].Выгрузить());
		ИначеЕсли РасхожденияКлиентСервер.ТипОснованияВозвратПоставщику(ТипОснованияАктаОРасхождении) Тогда
...
//Исходное	|				ТОГДА КорректировкаРеализации.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ИсправлениеОшибок)
//Глазунов
...
//ГлазуновДВ Добавили для выбора По ЗаказамКлиента	
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////5
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	РеализацияТоваровУслуг.Ссылка КАК ОформленнаяРеализация,
	|	РеализацияТоваровУслуг.Рин1_ДопоставкаПоРеализации КАК ДопоставкаПоРеализации
	|ИЗ
	|	Действия КАК Действия
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказКлиента КАК РеализацияТоваровУслуг
	|		ПО (РеализацияТоваровУслуг.Проведен)
	|			И Действия.Реализация = РеализацияТоваровУслуг.Рин1_ДопоставкаПоРеализации
//ГлазуновДВ Добавили для выбора По ЗаказамВнутреннегоПотребления
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////5
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|ИЗ
	|	Действия КАК Действия
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказНаВнутреннееПотребление КАК РеализацияТоваровУслуг
	|		ПО (РеализацияТоваровУслуг.Проведен)
	|			И Действия.Реализация = РеализацияТоваровУслуг.Рин1_ДопоставкаПоРеализации";
...
//Исходное				ВывестиДокументКорректировкаРеализации(СтрокаДействия, Перечисления.ХозяйственныеОперации.ИсправлениеОшибок, Макет, Описание);
			//{{ ГлазуновДВ
...
//Исходное			//	ВывестиДокументКорректировкаРеализации(СтрокаДействия, Перечисления.ХозяйственныеОперации.ИсправлениеОшибок, Макет, Описание);
			//{{20210120 ГлазуновДВ  ВЕРНУЛИ создание документа
				ВывестиДокументКорректировкаРеализации(СтрокаДействия, Перечисления.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон, Макет, Описание);
			//}} 20210120 ГлазуновДВ
...
//				ВывестиДокументРеализацияНаДопоставкуНедопоставленногоТовара(СтрокаДействия, Макет, Описание);
//ГлазуновДВ выводим ЗаказКлиента
				ВывестиДокументЗаказНаДопоставкуНедопоставленногоТовара(СтрокаДействия, Макет, Описание);
				ТабличныйДокумент.ЗакончитьГруппуСтрок();
...
//{{ГлазуновДВ		
		Если СтрокаДействия.ВариантыДействий.НайтиПоЗначению(Перечисления.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ОтгрузитьСейчас) <> Неопределено Тогда
			...
			Если РасхожденияКлиентСервер.ТипОснованияРеализацияТоваровУслуг(ТипОснованияАктаОРасхождении) Тогда
				...
				Описание.Вставить("АктОРасхождениях", АктОРасхождениях);
				Описание.Вставить("ИмяКоманды", "Оформить");
				Описание.Вставить("ВариантДействия", Перечисления.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ОтгрузитьСейчас);
				Описание.Вставить("СпособОтражения", СпособОтраженияРасхождений);
//ГлазуновДВ выводим ЗаказВнутреннегоПотребления
				ВывестиДокументЗаказНаДопоставкуНедопоставленногоТовара(СтрокаДействия, Макет, Описание);
				ТабличныйДокумент.ЗакончитьГруппуСтрок();
			КонецЕсли;
...
Процедура ВывестиДокументКорректировкаПриобретения(СтрокаДействия, Макет, Описание)
	...
	ЕстьДокументДляОформления = Ложь; //20201019 ГлазуновДВ
	Если ОформленныеДокументы.Количество() = 0 Тогда
...
//{{20201012 ГлазуновДВ Добавим проверку на все ли номенклатуры и количество оформлены по акту
		...
		Запрос.Текст = 
			...
			|	АктОРасхожденияхПослеПриемкиТовары.КоличествоУпаковок - АктОРасхожденияхПослеПриемкиТовары.КоличествоУпаковокПоДокументу - (ЕСТЬNULL(0 + КорректировкаПриобретенияРасхождения.КоличествоУпаковок, 0)) КАК КоличествоОсталось
			|	ИЗ
			|	Документ.АктОРасхожденияхПослеПриемки.Товары КАК АктОРасхожденияхПослеПриемкиТовары
			|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.КорректировкаПриобретения.Расхождения КАК КорректировкаПриобретенияРасхождения
			|			ПО (АктОРасхожденияхПослеПриемкиТовары.Ссылка = КорректировкаПриобретенияРасхождения.Ссылка.АктОРасхожденияхПослеПриемкиОснование)
			|				И (АктОРасхожденияхПослеПриемкиТовары.Номенклатура = КорректировкаПриобретенияРасхождения.Номенклатура)
			|	ГДЕ
			|		АктОРасхожденияхПослеПриемкиТовары.Ссылка = &АктОРасхождениях
//|		И АктОРасхожденияхПослеПриемкиТовары.Действие В (ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеПриемки.ОформитьПерепоставленноеИВернуть), ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеПриемки.ОформитьПерепоставленное))			
		...
		ЕстьДокументДляОформления = Ложь;
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			// Вставить обработку выборки ВыборкаДетальныеЗаписи
			Если ВыборкаДетальныеЗаписи.КоличествоОсталось <> 0 И (Строка(ВыборкаДетальныеЗаписи.Действие) = "Оформить"
				 ИЛИ Строка(ВыборкаДетальныеЗаписи.Действие) = "Оформить и вернуть") Тогда
				ЕстьДокументДляОформления = Истина;
			КонецЕсли;
		КонецЦикла;
		//}}КОНСТРУКТОР_ЗАПРОСА_С_ОБРАБОТКОЙ_РЕЗУЛЬТАТА
		
		Если ЕстьДокументДляОформления Тогда
			МассивОформленныхДокументов.Добавить(Документы.КорректировкаПриобретения.ПустаяСсылка());
		КонецЕсли;
//}}20201012 ГлазуновДВ
	...
	ВывестиДокументКОформлению(
		...
		МассивОформленныхДокументов,,,
//Исходное		Описание, );
		Описание, ЕстьДокументДляОформления);
...
//Исходное	                                 Описание = Неопределено)
	                                 Описание = Неопределено, ЕстьДоОформить = Ложь)      // ГлазуновДВ 20201019
	...
	//{{20201012-20201019 ГлазуновДВ	
		Если ЕстьДоОформить И ВыведеныОформленныеДокументы И Описание.ИмяКоманды = "Оформить" Тогда
			ОбластьСтрокаДокумента = Макет.ПолучитьОбласть(
				"СтрокаУровень" + Строка(Уровень) + ?(ИспользоватьГиперссылку, "", "БезГиперссылки"));
			ОбластьСтрокаДокумента.Параметры.Текст = ТекстГиперссылки;
...
//{{20200713 ГлазуновДВ
&НаСервере
Процедура ВывестиДокументЗаказНаДопоставкуНедопоставленногоТовара(СтрокаДействия, Макет, Описание)
	
	Если НеВыводитьВОтчет("ВывестиДокументЗаказНаДопоставкуНедопоставленногоТовара", СтрокаДействия.Основание) Тогда
		Возврат;
	КонецЕсли;
	...
	Если СтрокаДействия.ВариантыДействий.НайтиПоЗначению(Перечисления.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ТребуетсяДопоставка) <> Неопределено Тогда
		...
		ОформленныеДокументы = Допоставки1.НайтиСтроки(Новый Структура("ДопоставкаПоРеализации", СтрокаДействия.Основание));
		
		Если ОформленныеДокументы.Количество() = 0 Тогда
			
			МассивОформленныхДокументов.Добавить(Документы.ЗаказКлиента.ПустаяСсылка());
	...
	Если СтрокаДействия.ВариантыДействий.НайтиПоЗначению(Перечисления.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.ОтгрузитьСейчас) <> Неопределено Тогда
		...
		ОформленныеДокументы = Допоставки2.НайтиСтроки(Новый Структура("ДопоставкаПоРеализации", СтрокаДействия.Основание));
		
		Если ОформленныеДокументы.Количество() = 0 Тогда
			
			МассивОформленныхДокументов.Добавить(Документы.ЗаказНаВнутреннееПотребление.ПустаяСсылка());
...
Функция ТекстШапки(ТипДокумента, Описание , ХозяйственнаяОперация = Неопределено)
	...
	//Глазунов
		ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон Тогда
			ТекстШапки = НСтр("ru = 'Документ ""Корректировка реализации (корректировка по согласованию сторон)""';
								|en = 'Document ""Sale adjustment (error correction)""'");
	//Глазунов
		КонецЕсли;
	//Глазунов
	ИначеЕсли ТипДокумента = "РеализацияЗаказ" Тогда
		ТекстШапки = НСтр("ru = 'Документ ""Заказ клиента"" для допоставки(замены) товара';
							|en = 'Document ""Sales of goods and services"" for sales of underdelivered goods'");
	ИначеЕсли ТипДокумента = "РеализацияЗаказВнутренний" Тогда
		ТекстШапки = НСтр("ru = 'Документ ""Заказ на внутреннее потребление"" для допоставки товара';
							|en = 'Document ""Sales of goods and services"" for sales of underdelivered goods'");
	//Глазунов
...
Процедура ВыводитьЛегендуПриИзмененииНаСервере()
	...
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("Обработка.Рин1_РаботаСАктамиРасхождений", "ВыводитьЛегенду", ВыводитьЛегенду);
