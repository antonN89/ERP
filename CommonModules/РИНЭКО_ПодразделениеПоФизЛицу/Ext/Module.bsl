...
Процедура УстановитьПодразделениеСервер(ПодотчетноеЛицо,Подразделение,Организация) Экспорт
	...
	Для Каждого ЭлементСотрудник Из ТаблицаСотрудников Цикл 
		...
		Если ТаблицаКадровыхДанных.Количество() > 0 И ЗначениеЗаполнено(ТаблицаКадровыхДанных[0].Подразделение) И Организация = ТаблицаКадровыхДанных[0].Организация Тогда
			МассивДанныхДляПоискаПодразделения.Добавить(ТаблицаКадровыхДанных[0]);
		КонецЕсли;
		
	КонецЦикла;
	
	Если Не ЗначениеЗаполнено(МассивДанныхДляПоискаПодразделения) ИЛИ МассивДанныхДляПоискаПодразделения.Количество() > 1 Тогда
		Возврат;
	КонецЕсли;
