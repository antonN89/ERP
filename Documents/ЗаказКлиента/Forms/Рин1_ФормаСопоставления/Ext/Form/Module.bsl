...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СравнениеТоваров.Загрузить(ПолучитьИзВременногоХранилища(Параметры.Адрес_ТЗ_РасхожденияДляОтображения));
	СравнениеТоваровНеДляОтображения.Загрузить(ПолучитьИзВременногоХранилища(Параметры.Адрес_ТЗ_ТекущиеРасхождения));
...
Процедура ПолучитьАдресВоВременномХранилище(АдресВХранилище)
	...
	АдресВХранилище = ПоместитьВоВременноеХранилище(ТЗ_Вывод);
...
Функция ЗаполнитьСписокОтмен()
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Константа.Рин1_ПричинаОтменыПриРазделенииЗаказа КАК ПричинаОтменыПриРазделенииЗаказа
	|		ПО ПричиныОтменыЗаказовКлиентов.Ссылка <> ПричинаОтменыПриРазделенииЗаказа.Значение";
